...
import { WARNING_CODES, capTriggerSurface, coerceDescription, extractClaudeMdTriggers, firstNonEmptyLine, isSafePathComponent, isWithinRoot, loadManifest, readBody, readEnabledPluginIds, readFrontmatter, readMtime, scanSkillsDirEntries, splitDescriptionToPhrases, } from './local-inventory.helpers.js';
 ...
 */
function scanPluginSkills(skillsDir, manifest, warnings, pluginId) {
    return scanSkillsDirEntries(skillsDir, manifest, warnings).map((entry) => ({
        ...entry,
        origin: 'plugin',
        pluginId,
 ...
 */
function scanProjectSkills(skillsDir, manifest, warnings) {
    return scanSkillsDirEntries(skillsDir, manifest, warnings).map((entry) => ({
        ...entry,
 ...
 */
function scanPluginInventory(claudeDir, manifest, warnings) {
