...
async function resolveTeamId() {
    ...
    if (!licenseKey) {
        throw new Error('SKILLSMITH_LICENSE_KEY or SKILLSMITH_API_KEY is required for RBAC operations. Set one in ' +
    ...
    if (!teamId) {
        throw new Error('Unable to resolve team from the configured key. Ensure SKILLSMITH_LICENSE_KEY or ' +
...
function toToolError(err, requiredPermission) {
    const denied = toPermissionDeniedError(err, requiredPermission);
    if (denied)
        return denied;
    return err instanceof Error ? err.message : 'Unexpected RBAC error.';
}
const effectRow = (p) => `| ${p.role} | ${p.permission} | ${p.effect} | ${p.source} |`;
const PERMISSION_TABLE_HEAD = '| Role | Permission | Effect | Source |\n|------|------------|--------|--------|';
/** Expand `resources x actions` into `resource:action` strings, preserving input order. */
function expandPermissions(resources, actions) {
        ...
        switch (input.action) {
            case 'list_roles': {
                ...
                return {
                    ...
                    roles,
                    permissions,
                    message: `## Team Roles (${roles.length} configurable + owner)\n\n` +
        ...
        switch (input.action) {
            ...
            case 'revoke': {
                ...
                if (input.role === 'member')
                    return {
                        success: false,
                        dataSource,
                        error: 'Only "admin" can be revoked — that demotes the member to "member". Removing ' +
            ...
            case 'list_assignments': {
                ...
                return {
                    ...
                    dataSource,
                    assignments,
                    message: `## Team Members (${assignments.length})\n\n` +
                        (assignments.length === 0
                            ? 'No members found.'
                            : '| Member ID | Role | Name | Email |\n|-----------|------|------|-------|\n' +
                                assignments
                                    .map((a) => `| ${a.memberId} | ${a.role} | ${a.fullName ?? '—'} | ${a.email ?? '—'} |`)
                                    .join('\n')),
                };
            }
        }
    ...
    try {
        if (input.action === 'list') {
            ...
            return {
                ...
                dataSource,
                grants,
                message: `## Permission Overrides (${grants.length})\n\n` +
                    (grants.length === 0
        ...
        const unsupported = expanded.filter((p) => !TEAM_PERMISSIONS.includes(p));
        if (unsupported.length > 0)
            return {
                success: false,
                dataSource,
                error: `Not a configurable permission: ${unsupported.join(', ')}. ` +
        ...
        if (input.action === 'create') {
            ...
            if (failed.length > 0) {
                ...
                return {
                    ...
                    message: `## Policy Write Failed Partway Through${input.name ? `: ${input.name}` : ''}\n\n` +
                        `- **Role:** ${role}\n- **Effect:** ${effect}\n` +
            ...
            return {
                ...
                message: `## Policy Applied${input.name ? `: ${input.name}` : ''}\n\n` +
                    `- **Role:** ${role}\n- **Effect:** ${effect}\n` +
        ...
        for (const permission of permissions) {
            try {
                if (await svc.resetRolePermission(teamId, role, permission))
                    cleared += 1;
        ...
        if (resetFailed.length > 0) {
            ...
            return {
                ...
                message: `Cleared ${cleared} of ${resetSucceeded.length} attempted override(s) for role ` +
        ...
        return {
            success: true,
            dataSource,
            message: `Cleared ${cleared} of ${permissions.length} override(s) for role \`${role}\` ` +
...
//# sourceMappingURL=rbac-tools.action.js.map