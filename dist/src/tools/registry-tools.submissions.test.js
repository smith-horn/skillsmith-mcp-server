...
import { executePrivateRegistryPublish, executePrivateRegistryManage, createStubRegistryService, setPrivateRegistryService, } from './registry-tools.js';
const mockContext = {};
...
describe('private_registry_manage submissions/approve/reject — stub state machine', () => {
    it('lists a pending submission under "submissions" but not under "list"', async () => {
        ...
        const submissionsResult = await executePrivateRegistryManage({ action: 'submissions' }, mockContext);
    ...
    it('reject succeeds and the skill stays permanently invisible (D-8)', async () => {
        await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: SAMPLE_CONTENT }, mockContext);
        service.setActor(ADMIN_ACTOR);
        const result = await executePrivateRegistryManage({ action: 'reject', skillId: 'myteam/skill-a', version: '1.0.0' }, mockContext);
    ...
    it('blocks self-approval even when the submitter is also an admin (D-5 step 7 / D-6)', async () => {
        ...
        await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: SAMPLE_CONTENT }, mockContext);
        const result = await executePrivateRegistryManage({ action: 'approve', skillId: 'myteam/skill-a', version: '1.0.0' }, mockContext);
...
//# sourceMappingURL=registry-tools.submissions.test.js.map