...
import { CANONICAL_CLIENT, CLIENT_IDS, InvalidScopeValueError, parseInstallScope, removeLinks, resolveClientId, resolveScopedSkillsDir, UnsatisfiableWorkspaceScopeError, } from '@skillsmith/core/install';
...
export const uninstallInputSchema = z.object({
    ...
    scope: z
        ...
        .optional()
        .describe('Uninstall scope (ADR-139): "workspace" targets the nearest ancestor workspace ' +
    ...
    cwd: z
        ...
        .optional()
        .describe("Absolute path to the calling client's actual project/workspace root, used as the " +
 ...
 */
function buildScopeError(skillName, error) {
    return {
        success: false,
        skillName,
    ...
    inputSchema: {
        ...
        properties: {
            ...
            scope: {
                type: 'string',
                enum: ['global', 'workspace'],
                description: 'Uninstall scope (ADR-139): "workspace" targets the nearest ancestor workspace ' +
                    "marker/.git root's skills directory (walked from cwd when provided); defaults to " +
                    'SKILLSMITH_SCOPE env, then the per-client config default, then auto-detecting an ' +
                    'EXISTING workspace directory, then global.',
            },
            cwd: {
                type: 'string',
                description: "Absolute path to the calling client's actual project/workspace root, used as the " +
