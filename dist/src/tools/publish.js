...
// Import helpers
import { validateSkillPath, checkGhPreflight, generateChecksum, scanReferences, writeManifest, createGitHubRepo, addClaudeSkillTopic, } from './publish.helpers.js';
 ...
 */
async function executePublishImpl(input, _context) {
    // Validate input with Zod
    const validated = publishInputSchema.parse(input);
    const { skill_path, check_references, reference_patterns, create_repo, visibility, add_topic } = validated;
    ...
    if (check_references) {
        ...
        if (referenceWarnings.length > 0) {
            nextSteps.push(`Review ${referenceWarnings.length} project-specific reference(s) before publishing`);
        }
    }
    // Write manifest
        ...
        if (!preflight.ghAvailable) {
            ...
            if (create_repo) {
                ...
                if (url) {
                    ...
                    nextSteps.push(`Create GitHub repo manually: gh repo create ${metadata.name} --${visibility}`);
                }
            }
