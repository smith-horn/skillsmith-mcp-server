 ...
 */
function resolveRpc(kind, fn, params) {
    rpcCalls.push({ kind, fn, params: params ?? {} });
    if (fn !== 'check_registry_team_entitlement')
        return { data: null, error: null };
    if (entitlementRpcError)
        return { data: null, error: entitlementRpcError };
    ...
    if (!sub)
        return { data: { entitled: false, detail: 'subscription_not_found' }, error: null };
...
function createClient(kind) {
    return {
        rpc: async (fn, params) => resolveRpc(kind, fn, params),
        from: (table) => {
            const record = { table, op: 'select', columns: '', filters: {} };
            if (kind === 'user')
                userCalls.push(record);
            const chain = {
                select: (columns) => {
                ...
                then: (onFulfilled) => Promise.resolve(onFulfilled(resolve(kind, record))),
            };
            return chain;
        },
    };
...
const lastAuditMetadata = () => auditRows[auditRows.length - 1].metadata;
beforeEach(async () => {
    vi.clearAllMocks();
...
describe('getAdminUserClient / getMemberUserClient are never swapped at a call site', () => {
    ...
    it('setDeprecated() still uses the ADMIN getter — byte-for-byte the pre-Wave-3 message', async () => {
        const { resolveUserAccessToken } = await import('./team-resolver.js');
        vi.mocked(resolveUserAccessToken).mockResolvedValue(null);
        await expect(createLiveRegistryService().deprecate(TEAM_A, SKILL)).rejects.toThrow(/only team admins can deprecate/i);
        await expect(createLiveRegistryService().undeprecate(TEAM_A, SKILL)).rejects.toThrow(/only team admins can undeprecate/i);
    });
...
describe("getContent() entitlement is resolved against the ROW's own team", () => {
    it("caller is Enterprise via a DIFFERENT team but the row's team is downgraded → denied, " +
        'and profiles.tier is never consulted', async () => {
        subsById['sub-a'] = { tier: 'team', status: 'active' };
        subsById['sub-b'] = { tier: 'enterprise', status: 'active' };
        await expect(createLiveRegistryService().getContent(TEAM_A, SKILL)).rejects.toThrow(/requires an active Enterprise subscription on the team that owns it/i);
    ...
    it.each([
        ...
        mutate();
        await expect(createLiveRegistryService().getContent(TEAM_A, SKILL)).rejects.toThrow(/Enterprise/i);
        expect(contentQueries).toBe(0);
        expect(lastAuditMetadata().detail).toBe(detail);
    });
    it.each(['active', 'trialing', 'past_due'])('accepts status "%s" — the same whitelist recompute_user_tier uses', async (status) => {
    ...
    it('surfaces an entitlement-lookup outage as an error, never as "not entitled"', async () => {
        ...
        await expect(createLiveRegistryService().getContent(TEAM_A, SKILL)).rejects.toThrow(/Entitlement lookup failed: connection refused/);
        expect(contentQueries).toBe(0);
...
describe('getContent() visibility + version selection', () => {
    ...
    it('surfaces a failed metadata query as an error, never as "not found"', async () => {
        ...
        await expect(createLiveRegistryService().getContent(TEAM_A, SKILL)).rejects.toThrow(/Failed to read registry skill content/i);
        expect(auditRows[auditRows.length - 1].result).toBe('error');
        expect(lastAuditMetadata().detail).toBe('PGRST301');
    });
});
//# sourceMappingURL=registry-tools.live.content.test.js.map