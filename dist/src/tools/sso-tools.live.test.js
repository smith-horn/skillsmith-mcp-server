...
describe('createLiveSSOService — team-sso-manage fetch() mapping', () => {
    ...
    it('refuses "set" locally without a domain, without calling fetch', async () => {
        const result = await executeConfigureSso({ action: 'set', idpMetadataUrl: 'https://idp.example.com/metadata', protocol: 'saml' }, mockContext);
        expect(result.success).toBe(false);
    ...
    it('400 invalid_role_mapping -> a plain validation string, not a permission denial', async () => {
        fetchMock.mockResolvedValue(jsonResponse({ error: 'invalid_role_mapping', message: 'role_mapping may not name "owner".' }, 400));
        const result = await executeConfigureSso({
            action: 'set',
            idpMetadataUrl: 'https://idp.example.com/metadata',
            protocol: 'saml',
    ...
    it('409 domain_verified_by_another_team -> a plain string, no domainNotVerified', async () => {
        ...
        const result = await executeConfigureSso({ action: 'verify_domain', domain: 'example.com', protocol: 'saml' }, mockContext);
        expect(result.success).toBe(false);
        expect(result.error).toBe('This domain is already verified by another team.');
        expect(result.domainNotVerified).toBeUndefined();
    });
    it('never leaks a GoTrue-native raw field (msg/error_code) even when the response body carries ' +
        'one alongside our own authored error/message', async () => {
        fetchMock.mockResolvedValue(jsonResponse({
            error: 'domain_verified_by_another_team',
            message: 'This domain is already verified by another team.',
            ...
            msg: 'duplicate key value violates unique constraint "sso_providers_pkey": ' +
    ...
    it.each([500, 502, 503])('%d -> a fixed generic message, never the raw response body text', async (status) => {
        fetchMock.mockResolvedValue(jsonResponse({ msg: 'internal GoTrue stack trace, never to be shown to a customer' }, status));
        const result = await executeConfigureSso({ action: 'test', protocol: 'saml' }, mockContext);
        expect(result.success).toBe(false);
    ...
    it('claim_domain: maps a live response into domainClaim with no simulated flag', async () => {
        ...
        const result = await executeConfigureSso({ action: 'claim_domain', domain: 'example.com', protocol: 'saml' }, mockContext);
        expect(result.success).toBe(true);
    ...
    it('verify_domain: a successful verification reports verified: true with no simulated flag', async () => {
        ...
        fetchMock.mockResolvedValue(jsonResponse({ ok: true, domain: 'example.com', verifiedAt: '2026-08-28T00:00:00Z' }, 200));
        const result = await executeConfigureSso({ action: 'verify_domain', domain: 'example.com', protocol: 'saml' }, mockContext);
        expect(result.success).toBe(true);
    ...
    it('verify_domain: a DNS mismatch/not-found result (409) renders the retry message, not success', async () => {
        fetchMock.mockResolvedValue(jsonResponse({
            ...
            message: 'No TXT record found at "_skillsmith-verify.example.com". Publish it and try again.',
        }, 409));
        const result = await executeConfigureSso({ action: 'verify_domain', domain: 'example.com', protocol: 'saml' }, mockContext);
        expect(result.success).toBe(false);
...
//# sourceMappingURL=sso-tools.live.test.js.map