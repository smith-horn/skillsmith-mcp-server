 ...
 */
const SNAPSHOT_PATH = process.env.SKILLSMITH_TYPOSQUAT_SNAPSHOT_PATH ||
    join(getAssetsDir(), 'typosquat-reference-snapshot.json');
let cachedReferenceNames;
/**
...
function loadReferenceNames() {
    if (cachedReferenceNames)
        return cachedReferenceNames;
    try {
        const raw = readFileSync(SNAPSHOT_PATH, 'utf-8');
        const parsed = JSON.parse(raw);
        cachedReferenceNames = new Set(Array.isArray(parsed.names) ? parsed.names.map((n) => n.toLowerCase()) : []);
    }
    catch {
 ...
 */
export function detectTyposquatInName(skillName) {
 ...
 * @param skillName the skill's own declared name (SKILL.md frontmatter `name`)
 */
export function scanTyposquatName(skillName) {
    return detectTyposquatInName(skillName).map((finding) => ({
        field: 'name',
        message: `Possible typosquat: ${finding.message} ` +
            `See https://skillsmith.app/docs/security/scanner#typosquat`,
        severity: 'warning',
    }));
}
//# sourceMappingURL=validate-typosquat-scan.js.map