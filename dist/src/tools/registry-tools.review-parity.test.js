...
function expectOnlyPattern(message, expected) {
...
function liveRpcError(error) {
    return {
        rpcResponder: (fn) => fn === 'review_private_registry_submission'
            ? { data: null, error }
            : { data: [], error: null },
    };
}
describe('stub/live review-gate error parity (SMI-5949 Wave 2 Step 5, M7)', () => {
    ...
    it('not-admin: both transports fail at the admin-membership check (D-5 step 3)', async () => {
        const { client } = createFakeClient(liveRpcError({
            code: '42501',
        ...
        const liveErr = await live
            ...
            .catch((e) => e);
        expectOnlyPattern(liveErr.message, 'notAdmin');
        ...
        const stubErr = await stub
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(stubErr.message, 'notAdmin');
    });
    it('self-approval: both transports fail at self-approval, not the admin check (D-5 step 7 / D-6)', async () => {
        ...
        const liveErr = await live
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(liveErr.message, 'selfApproval');
        ...
        const stubErr = await stub
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(stubErr.message, 'selfApproval');
    });
    it('already-decided: both transports fail at the terminal-state check (D-5 step 5 / D-8)', async () => {
        const { client } = createFakeClient(liveRpcError({
            ...
            message: 'this submission is already approved -- an approval decision is final. Versions are ' +
        ...
        const liveErr = await live
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(liveErr.message, 'terminal');
        ...
        const stubErr = await stub
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(stubErr.message, 'terminal');
    });
    it('missing published_by: both transports fail at the legacy-client check (D-5 step 6, D-7)', async () => {
        ...
        const liveErr = await live
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(liveErr.message, 'missingPublisher');
        ...
        const stubErr = await stub
            .review(REVIEW_TEAM, REVIEW_SKILL, REVIEW_VERSION, 'approved')
            .catch((e) => e);
        expectOnlyPattern(stubErr.message, 'missingPublisher');
    });
});
//# sourceMappingURL=registry-tools.review-parity.test.js.map