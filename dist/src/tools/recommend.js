 ...
 */
async function searchLocalSkillsForRecommend(query, limit) {
    try {
        ...
        console.warn('[skillsmith] Local skill search for recommend failed:', error.message);
        return [];
    }
}
/**
 ...
 */
async function executeRecommendImpl(input, context) {
    const startTime = performance.now();
    // Validate input with Zod
    const validated = recommendInputSchema.parse(input);
    let { installed_skills } = validated;
    const { project_context, limit, detect_overlap, min_similarity, role, installable_only } = validated;
    ...
    if (!context.apiClient.isOffline()) {
        try {
            ...
            let recommendations = mergeAndDeduplicateRecommendations(apiRecommendations, localRecommendations, limit);
            const endTime = performance.now();
            // Filter out already installed skills
            recommendations = recommendations.filter((rec) => !installed_skills.some((id) => id.toLowerCase() === rec.skill_id.toLowerCase()));
            ...
            const apiCandidates = apiResultSettled.status === 'fulfilled' ? apiResultSettled.value.data.length : 0;
    ...
    const localRecommendations = localSkillsResult.status === 'fulfilled' ? localSkillsResult.value : [];
