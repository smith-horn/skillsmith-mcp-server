...
// mark_local
// ============================================================================
export async function runMarkLocal(input, scopeTarget, _context) {
    ...
    await withLockTimeoutMapping(scopeTarget.manifestPath, () => manager.updateSafely((manifest) => {
        ...
        const updated = { ...entry, source: 'unknown', provenance: 'local' };
        afterState = updated;
        return {
            ...manifest,
...
// relink
// ============================================================================
export async function runRelink(input, scopeTarget, context) {
    const name = requireName(input);
    ...
    await withLockTimeoutMapping(scopeTarget.manifestPath, () => manager.updateSafely((manifest) => {
        ...
        // change together.
        const updated = { ...entry, id, source, provenance: 'registry' };
...
// drop_entry
// ============================================================================
export async function runDropEntry(input, scopeTarget, _context) {
    const name = requireName(input);
    const manager = new ManifestManager(scopeTarget.manifestPath);
    const { entry: preCheckEntry } = resolveReconcileEntry(await manager.load(), name, scopeTarget.client);
...
// revert (C7)
// ============================================================================
export async function runRevert(input, scopeTarget, _context) {
    ...
    await withLockTimeoutMapping(target.manifestPath, () => manager.updateSafely((manifest) => {
        ...
        updatedSkills[key] = target.beforeState;
...
//# sourceMappingURL=apply-manifest-reconcile.actions.js.map