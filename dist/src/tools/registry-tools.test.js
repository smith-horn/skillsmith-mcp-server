...
import { privateRegistryPublishInputSchema, privateRegistryManageInputSchema, executePrivateRegistryPublish, executePrivateRegistryManage, createStubRegistryService, setPrivateRegistryService, } from './registry-tools.js';
const mockContext = {};
...
describe('registry-tools', () => {
    ...
    async function approve(skillId, version) {
        service.setActor(ADMIN_ACTOR);
        const result = await executePrivateRegistryManage({ action: 'approve', skillId, version }, mockContext);
        if (!result.success)
    ...
    describe('privateRegistryPublishInputSchema', () => {
        ...
        it('should reject missing content', () => {
            expect(() => privateRegistryPublishInputSchema.parse({ skillId: 'myteam/my-skill', version: '1.0.0' })).toThrow();
        });
        it('should reject invalid skill ID format', () => {
            expect(() => privateRegistryPublishInputSchema.parse({
                skillId: 'no-slash',
                version: '1.0.0',
        ...
        it.each(['myteam/..', 'myteam/.', '../myteam', 'myteam/   '])('should reject skillId "%s" (unsafe path segment)', (skillId) => {
            expect(() => privateRegistryPublishInputSchema.parse({
                skillId,
                version: '1.0.0',
                content: SAMPLE_CONTENT,
    ...
    describe('privateRegistryManageInputSchema', () => {
        ...
        it.each(['myteam/..', 'myteam/.', '../myteam'])('should reject skillId "%s" (unsafe path segment) for the install action', (skillId) => {
            expect(() => privateRegistryManageInputSchema.parse({ action: 'install', skillId })).toThrow();
        });
    });
    // ==========================================================================
    ...
    describe('executePrivateRegistryManage', () => {
        ...
        it('should not list a freshly published (still-pending) skill — SMI-5949 D-4', async () => {
            await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: SAMPLE_CONTENT }, mockContext);
        ...
        it('should get a specific skill once approved', async () => {
            await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: SAMPLE_CONTENT }, mockContext);
            await approve('myteam/skill-a', '1.0.0');
            const result = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/skill-a' }, mockContext);
            expect(result.success).toBe(true);
            expect(result.skill).toBeDefined();
        ...
        it('should fail get for nonexistent skill, with a generic non-leaking hint (SMI-5949 M11)', async () => {
            const result = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/nonexistent' }, mockContext);
            expect(result.success).toBe(false);
        ...
        it('should deprecate a skill', async () => {
            await executePrivateRegistryPublish({ skillId: 'myteam/old-skill', version: '1.0.0', content: SAMPLE_CONTENT }, mockContext);
            ...
            const result = await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/old-skill' }, mockContext);
            expect(result.success).toBe(true);
            ...
            const getResult = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/old-skill' }, mockContext);
            ...
            // flipped.
            const listResult = await executePrivateRegistryManage({ action: 'list', includeDeprecated: true }, mockContext);
            expect(listResult.skills.find((s) => s.skillId === 'myteam/old-skill')?.deprecated).toBe(true);
        });
        ...
        it('should fail deprecate for nonexistent skill', async () => {
            const result = await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/nonexistent' }, mockContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain('not found');
        });
        it('should undeprecate a skill', async () => {
            await executePrivateRegistryPublish({ skillId: 'myteam/revived', version: '2.0.0', content: SAMPLE_CONTENT }, mockContext);
            ...
            await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/revived' }, mockContext);
            const result = await executePrivateRegistryManage({ action: 'undeprecate', skillId: 'myteam/revived' }, mockContext);
            expect(result.success).toBe(true);
            expect(result.message).toContain('undeprecated');
            // Verify it's no longer deprecated
            const getResult = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/revived' }, mockContext);
        ...
        describe('deprecated read-filter closure (SMI-5949 Wave 3)', () => {
            it('a deprecated skill is hidden from list by default', async () => {
                await executePrivateRegistryPublish({ skillId: 'myteam/old-skill', version: '1.0.0', content: SAMPLE_CONTENT }, mockContext);
                await approve('myteam/old-skill', '1.0.0');
                await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/old-skill' }, mockContext);
                const result = await executePrivateRegistryManage({ action: 'list' }, mockContext);
                expect(result.success).toBe(true);
                expect(result.skills).toHaveLength(0);
            });
            it('includeDeprecated:true reveals it again via list', async () => {
                ...
                await approve('myteam/old-skill', '1.0.0');
                await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/old-skill' }, mockContext);
                const result = await executePrivateRegistryManage({ action: 'list', includeDeprecated: true }, mockContext);
                expect(result.success).toBe(true);
                expect(result.skills).toHaveLength(1);
            ...
            it('get has no includeDeprecated opt-in — a deprecated skill is not found via get regardless', async () => {
                ...
                await approve('myteam/old-skill', '1.0.0');
                await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/old-skill' }, mockContext);
                const result = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/old-skill' }, mockContext);
                expect(result.success).toBe(false);
                expect(result.error).toContain('not found');
            });
        });
...
//# sourceMappingURL=registry-tools.test.js.map