...
describe('installInputSchema client/alsoLink enum (SMI-5982 Wave 6)', () => {
    it.each([...CLIENT_IDS])('accepts client=%s (every current ClientId, not just the original 5)', (client) => {
...
describe('installInputSchema cwd field validation (SMI-5982 PR-review follow-up)', () => {
    it('rejects a relative cwd with a clear message', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
            expect(result.error.issues.some((i) => i.message === 'cwd must be an absolute path')).toBe(true);
        }
    });
...
//# sourceMappingURL=install.types.test.js.map