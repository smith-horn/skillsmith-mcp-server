...
const DEFAULT_ACTOR = { id: 'stub-actor-1', isAdmin: true };
function toRegistrySkill(teamId, row) {
    return {
        skillId: row.skillId,
        version: row.version,
...
export function createStubRegistryService() {
    // Keyed by `${teamId}::${skillId}` so the stub never leaks across teams.
    const registry = new Map();
    const key = (teamId, skillId) => `${teamId}::${skillId}`;
    // Every published version's content, keyed by `${teamId}::${skillId}::${version}`.
    const versions = new Map();
    const versionKey = (teamId, skillId, version) => `${key(teamId, skillId)}::${version}`;
    ...
    function latestVersion(teamId, skillId) {
    ...
    return {
        ...
        async get(teamId, skillId, version) {
            ...
            if (!row || row.approvalStatus !== 'approved' || row.deprecated)
                return null;
        ...
        async getContent(teamId, skillId, version) {
            ...
            if (!metadata || metadata.approvalStatus !== 'approved' || metadata.deprecated)
                return null;
            const entry = version
        ...
        async review(teamId, skillId, version, decision, note) {
            ...
            if (decision !== 'approved' && decision !== 'rejected') {
                throw new Error(`Invalid decision "${String(decision)}" — must be "approved" or "rejected".`);
            }
            ...
            if (row.approvalStatus !== 'pending') {
                throw new Error(`This submission has already been ${row.approvalStatus} and cannot be reviewed again ` +
            ...
            if (row.publishedBy === null) {
                throw new Error('This submission has no recorded submitter (published_by is NULL) and cannot be ' +
            ...
            if (row.publishedBy === actor.id) {
                throw new Error('You cannot approve your own submission. Ask another team admin to review it.');
            }
