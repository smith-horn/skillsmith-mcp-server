...
// verify (C3)
// ============================================================================
export async function runVerify(input, scopeTarget, context) {
    ...
    for (const { key, entry } of targets) {
        const outcome = await verifyEntryAgainstRegistry(entry, context, () => {
            quotaExhausted = true;
        }, quotaExhausted);
    ...
    for (const { key, verifiedAt, priorEntry } of actuallyWritten) {
        const ledgerEntry = await appendReconcileLedgerEntry({
            ...
            action: 'verify',
            beforeState: priorEntry,
            afterState: { ...priorEntry, verifiedAt },
...
//# sourceMappingURL=apply-manifest-reconcile.verify.js.map