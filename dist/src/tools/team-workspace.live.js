...
import { getWorkspaceManageUserClient, getWorkspaceMemberUserClient, } from './team-workspace.live.auth.js';
...
export function createLiveService() {
    return {
        async resolveTeamId(licenseKey) {
            ...
            if (!teamId) {
                throw new Error('Unable to resolve team from the configured key. Ensure SKILLSMITH_LICENSE_KEY or SKILLSMITH_API_KEY (SMI-6080) is set and corresponds to an active Team-tier subscription.');
            }
            return teamId;
        },
        ...
        async deleteWorkspace(teamId, workspaceId) {
            ...
            const probe = await client
                ...
                .eq('team_id', teamId);
            if (probe.error) {
                throw new Error(`Failed to delete workspace: the delete matched no rows and the follow-up check also ` +
        ...
        async removeSkill(teamId, workspaceId, skillId) {
            ...
            const probe = await client
                ...
                .eq('skill_id', skillId);
            if (probe.error) {
                throw new Error(`Failed to remove skill: the delete matched no rows and the follow-up check also ` +
                    `failed, so we cannot tell whether it was already removed or you lost access: ` +
                    `${probe.error.message ?? 'unknown error'}`);
            }
            if (Array.isArray(probe.data) && probe.data.length > 0) {
                throw new Error(`Could not remove "${skillId}" from this workspace even though it is shared there — ` +
