...
export function generateDailyTrend(days) {
    const trend = [];
    const now = new Date();
...
export function stubTeamAnalyticsDashboard(period) {
...
export function stubTeamUsageReport(period, format) {
    const days = periodDays(period);
    ...
    if (format === 'detailed') {
        lines.push('', '## Detailed Breakdown by User', '| User | Discovery | Management | Quality | Collaboration | Total |', '|------|-----------|------------|---------|---------------|-------|', `| alice@example.com | 95 | 60 | 30 | 15 | ${Math.round(totalCalls * 0.32)} |`, `| bob@example.com | 80 | 70 | 20 | 5 | ${Math.round(totalCalls * 0.28)} |`, `| carol@example.com | 65 | 45 | 35 | 40 | ${Math.round(totalCalls * 0.24)} |`, `| dave@example.com | 40 | 30 | 25 | 5 | ${Math.round(totalCalls * 0.16)} |`);
    }
    return lines.join('\n');
}
/** Stub fallback for enterprise analytics dashboard */
export function stubAnalyticsDashboard(period, includeRecommendations) {
    const days = periodDays(period);
    ...
    if (includeRecommendations) {
        lines.push('', '## Recommendation Accuracy', `- **Recommendations made**: ${Math.round(totalCalls * 0.18)}`, `- **Accepted**: ${Math.round(totalCalls * 0.18 * 0.72)} (72%)`, `- **Installed after recommendation**: ${Math.round(totalCalls * 0.18 * 0.45)} (45%)`, `- **Still active after 7 days**: ${Math.round(totalCalls * 0.18 * 0.38)} (38%)`, '', '## Top Recommended Skills', '| Skill | Times Recommended | Accept Rate |', '|-------|-------------------|-------------|', '| governance | 42 | 85% |', '| security-auditor | 35 | 74% |', '| flaky-test-detector | 28 | 68% |');
    }
    return lines.join('\n');
}
/** Stub fallback for enterprise usage report */
export function stubUsageReport(period, format) {
    const days = periodDays(period);
    const totalCalls = days * 85;
    ...
    if (format === 'detailed') {
        lines.push('', '## Per-Team Detailed Breakdown', '', '### Engineering (10 users)', '| User | Total | search | install | validate | audit |', '|------|-------|--------|---------|----------|-------|', '| eng-lead | 320 | 120 | 80 | 60 | 60 |', '| dev-1 | 280 | 100 | 90 | 50 | 40 |', '| dev-2 | 240 | 90 | 70 | 40 | 40 |', '', '### Data Science (5 users)', '| User | Total | search | recommend | compare | suggest |', '|------|-------|--------|-----------|---------|---------|', '| ds-lead | 210 | 70 | 80 | 30 | 30 |', '| analyst-1 | 180 | 60 | 60 | 30 | 30 |');
    }
    return lines.join('\n');
}
//# sourceMappingURL=analytics.stub.js.map