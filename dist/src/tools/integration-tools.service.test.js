...
describe('createRealIntegrationService', () => {
    describe('createWebhook', () => {
        ...
        it('should reject private IP URLs (SSRF)', async () => {
            ...
            const svc = createRealIntegrationService(supabase, TEAM_ID);
            await expect(svc.createWebhook('https://192.168.1.1/hook', ['skill.install'])).rejects.toThrow('Invalid webhook URL');
        });
        it('should reject HTTP URLs', async () => {
            const { supabase } = createMockSupabase();
            const svc = createRealIntegrationService(supabase, TEAM_ID);
            await expect(svc.createWebhook('http://example.com/hook', ['skill.install'])).rejects.toThrow('Invalid webhook URL');
        });
        it('should throw on Supabase error', async () => {
            ...
            const svc = createRealIntegrationService(supabase, TEAM_ID);
            await expect(svc.createWebhook('https://example.com/hook', ['skill.install'])).rejects.toThrow('Failed to create webhook');
        });
    });
    // ==========================================================================
    // listWebhooks
...
//# sourceMappingURL=integration-tools.service.test.js.map