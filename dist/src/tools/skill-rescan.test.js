...
import { SkillDependencyRepository, QuarantineRepository } from '@skillsmith/core';
...
`;
    ...
    it('contains a dependency-persistence failure to the affected skill only', async () => {
        ...
        const good = result.results.find((r) => r.skill === 'good-skill');
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const throwsEntry = result.results.find((r) => r.skill === 'throws-skill');
        expect(good?.passed).toBe(true);
...
`;
    ...
    it('includes totalDependenciesBackfilled: 0 on the "skill not found" error path', async () => {
        ...
        const result = await executeSkillRescan({ skillName: 'nonexistent' }, skillsDir, undefined, depRepo);
        expect(result.error).toBeDefined();
        expect(result.totalDependenciesBackfilled).toBe(0);
    });
});
...
describe('executeSkillRescan → SMI-5207: un-sticking an already-flagged skill (severityCounts)', () => {
    ...
    function icmShipwrightFixture(name) {
        return `---
name: ${name}
...
`;
    ...
    it('rescanning an already-quarantined (stale pre-fix HIGH) FP-shaped skill reports the current, lower severity — not the stale one', async () => {
        ...
        expect(entry.topFindings
            .filter((f) => f.type === 'sensitive_path')
            .every((f) => f.severity !== 'high' && f.severity !== 'critical')).toBe(true);
...
`;
        ...
        expect(entry.topFindings.some((f) => f.type === 'sensitive_path' && f.severity === 'high')).toBe(false);
        expect(entry.topFindings.some((f) => f.type === 'prompt_leaking' && f.severity === 'critical')).toBe(true);
