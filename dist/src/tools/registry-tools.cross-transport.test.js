...
import { SkillInstallationService, SkillRepository, SkillDependencyRepository, createDatabaseAsync, initializeSchema, getPrivateRegistrySkillContent, } from '@skillsmith/core';
import { executeRegistryInstall } from './registry-tools.install-action.js';
import { createStubRegistryService } from './registry-tools.js';
const mockContext = {};
...
async function makeRig(label) {
    ...
    initializeSchema(db);
    const tmpDir = path.join(os.tmpdir(), `skillsmith-registry-${label}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    const skillsDir = path.join(tmpDir, 'skills');
    await fs.mkdir(skillsDir, { recursive: true });
    return { db, skillsDir, manifestPath: path.join(tmpDir, 'manifest.json') };
}
function installerFor(rig) {
...
function installViaMcp(service, rig, version) {
 ...
 * fixture mismatch.
 */
async function installViaCli(service, rig, version) {
    ...
    if (!fetchResult.ok)
        return { fetchResult, installResult: null };
...
describe('CLI transport — full publish -> install round trip (real client fn + real installer)', () => {
    it('installs published content to disk with private-registry provenance', async () => {
        ...
        const installedSkillMd = await fs.readFile(path.join(cliRig.skillsDir, 'acme-tool', 'SKILL.md'), 'utf-8');
    ...
    it('a deprecated skill still installs via the install path itself, not just the metadata fetch', async () => {
        ...
        expect(installResult?.success).toBe(true);
        await expect(fs.access(path.join(cliRig.skillsDir, 'acme-tool', 'SKILL.md'))).resolves.toBeUndefined();
    });
});
// ---------------------------------------------------------------------------
...
describe('MCP and CLI transports agree on version selection, given the same published data', () => {
    ...
    it('an omitted version resolves to the identical most-recently-published row on both transports', async () => {
        ...
        expect(installResult?.success).toBe(true);
        const mcpSkillMd = await fs.readFile(path.join(mcpRig.skillsDir, 'acme-tool', 'SKILL.md'), 'utf-8');
        const cliSkillMd = await fs.readFile(path.join(cliRig.skillsDir, 'acme-tool', 'SKILL.md'), 'utf-8');
...
//# sourceMappingURL=registry-tools.cross-transport.test.js.map