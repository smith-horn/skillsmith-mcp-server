...
function manifestWithSkills(entries) {
 ...
 */
function insertVersionAt(db, skillId, contentHash, semver, recordedAt) {
    db.prepare(`INSERT OR IGNORE INTO skill_versions (skill_id, content_hash, recorded_at, semver)
     VALUES (?, ?, ?, ?)`).run(skillId, contentHash, recordedAt, semver);
}
// ============================================================================
// Tests
// ============================================================================
describe('executeSkillUpdates', () => {
    ...
    it('bounds the default skillIds to the manifest, ignoring registry-wide skill_versions rows', async () => {
        ...
        mockedLoadManifest.mockResolvedValue(manifestWithSkills([{ key: 'astro', id: 'community/astro', contentHash: 'oldoldold1' }]));
    ...
    it('honors an explicit skillIds filter, still consulting the manifest for the real installed hash (SMI-6343 C1)', async () => {
        ...
        // hashes instead.
        mockedLoadManifest.mockResolvedValue(manifestWithSkills([{ key: 'explicit', id: 'explicit/skill', contentHash: 'hashhash01' }]));
    ...
    it('skips a manifest-scoped id with no version history instead of erroring', async () => {
        mockedLoadManifest.mockResolvedValue(manifestWithSkills([{ key: 'no-history', id: 'community/no-history' }]));
    ...
    it('reports updateAvailable: false when the manifest-recorded hash matches the latest registry hash', async () => {
        mockedLoadManifest.mockResolvedValue(manifestWithSkills([{ key: 'stable', id: 'community/stable', contentHash: 'samehash01' }]));
...
//# sourceMappingURL=skill-updates.test.js.map