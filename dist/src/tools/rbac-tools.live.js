 ...
 */
function mapEffectivePermissionRow(row) {
    return {
        role: row.role,
 ...
 */
export function createLiveRBACService() {
    return {
        ...
        async listPermissions(teamId) {
            ...
            const resp = await client.rpc('get_effective_team_permissions', {
                p_team_id: teamId,
            });
            if (resp.error) {
        ...
        async setRolePermission(teamId, role, permission, effect) {
            ...
            const resp = await client.rpc('set_team_role_permission', {
                p_team_id: teamId,
                p_role: role,
        ...
        async resetRolePermission(teamId, role, permission) {
            ...
            const resp = await client.rpc('reset_team_role_permission', {
                p_team_id: teamId,
                p_role: role,
                p_permission: permission,
            });
            ...
            if (resp.data === null) {
                throw new Error('reset_team_role_permission reported success but returned no value — this should ' +
        ...
        async listMembers(teamId) {
            ...
            const resp = await client.rpc('list_team_members_with_profile', {
                p_team_id: teamId,
            });
            if (resp.error) {
        ...
        async setMemberRole(memberId, role) {
            ...
            const resp = await client.rpc('set_team_member_role', {
...
//# sourceMappingURL=rbac-tools.live.js.map