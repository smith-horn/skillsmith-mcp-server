 ...
 */
function createRecorder(respond = () => ({
    data: [{ id: 'row-1' }],
    error: null,
})) {
    const calls = [];
    function makeQuery(table) {
        ...
        const chain = {
            ...
            then: (onFulfilled) => Promise.resolve(onFulfilled(respond(record))),
        };
        return chain;
    }
    return {
...
async function setClients(userClient, adminClient) {
...
describe('SMI-5822 — deprecate/undeprecate require a user credential, not the team license key', () => {
    ...
    it.each([
        { op: 'deprecate', expected: true },
        { op: 'undeprecate', expected: false },
    ])('$op issues its UPDATE on the user client — the service-role client never updates the table', async ({ op, expected }) => {
        ...
        const userUpdate = user.calls.find((c) => c.op === 'update' && c.table === 'private_registry_skills');
        ...
        expect(userUpdate.filters).toEqual(expect.arrayContaining([
            { column: 'team_id', value: TEAM },
            { column: 'skill_id', value: SKILL },
        ]));
        // This is the assertion that inverts Wave 1's block E2.
        expect(admin.calls.some((c) => c.op === 'update' && c.table === 'private_registry_skills')).toBe(false);
    });
    it.each(['deprecate', 'undeprecate'])('%s refuses — and writes nothing — when no user is signed in', async (op) => {
        const { resolveUserAccessToken } = await import('./team-resolver.js');
    ...
    it.each(['deprecate', 'undeprecate'])('%s tells a non-admin member they are not an admin, rather than "not found"', async (op) => {
        ...
        const user = createRecorder((record) => record.op === 'update'
            ? { data: [], error: null }
    ...
    it.each(['deprecate', 'undeprecate'])('%s still reports not-found when the skill is not even readable', async (op) => {
    ...
    // ==========================================================================
    it.each(['deprecate', 'undeprecate'])('%s surfaces a failed readability probe as an error instead of reporting "not found"', async (op) => {
        const user = createRecorder((record) => record.op === 'update'
            ? { data: [], error: null }
        ...
        expect((audit.payload?.metadata).detail).toBe('PGRST301');
    });
    ...
    it('attributes an admin-authorized deprecate to the JWT user, not to the license key', async () => {
        ...
        const metadata = audit.payload?.metadata;
    ...
    it('records an explicitly-unattributed actor when the user token yields no subject', async () => {
        ...
        expect((audit.payload?.metadata).actor_user_id).toBeNull();
    });
    ...
    it('attributes an authorized publish to the JWT user, not to the license key — SMI-5949 D-7', async () => {
        ...
        const userInsert = user.calls.find((c) => c.op === 'insert' && c.table === 'private_registry_skills');
        expect(userInsert).toBeDefined();
        expect(admin.calls.some((c) => c.op === 'insert' && c.table === 'private_registry_skills')).toBe(false);
        const audit = admin.calls.find((c) => c.table === 'audit_logs' && c.op === 'insert');
        expect(audit).toBeDefined();
    ...
    it('publish refuses — and writes nothing — when no user is signed in (D-7)', async () => {
        ...
        const admin = createRecorder();
        await setClients(user.client, admin.client);
        await expect(createLiveRegistryService().publish(TEAM, SKILL, '1.0.0', { 'SKILL.md': '# a skill' })).rejects.toThrow(/skillsmith login/i);
...
//# sourceMappingURL=registry-tools.live.admin-auth.test.js.map