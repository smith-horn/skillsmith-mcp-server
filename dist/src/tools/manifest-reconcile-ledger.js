...
import { CURRENT_LEDGER_VERSION, } from './manifest-reconcile-ledger.types.js';
 ...
 */
export async function readReconcileLedgerResult(opts = {}) {
 ...
 */
export async function readReconcileLedger(opts = {}) {
    const result = await readReconcileLedgerResult(opts);
    switch (result.kind) {
        case 'ok':
            return result.ledger;
        case 'manifest.reconcile.ledger_malformed':
            console.warn(`[manifest-reconcile-ledger] ledger malformed (${result.reason}); using empty ledger`);
            return emptyLedger();
        case 'manifest.reconcile.ledger_version_unsupported': {
            const err = new Error(`manifest-reconcile-ledger version ${String(result.found)} is newer than supported version ${String(result.expected)}`);
 ...
 */
export async function writeReconcileLedger(ledger, opts = {}) {
    const ledgerPath = opts.ledgerPath ?? defaultLedgerPath();
    ...
    try {
        ...
        try {
            ...
            console.warn(`[manifest-reconcile-ledger] failed to remove stale tmp file '${tmpPath}': ${cleanupErr.message}`);
        }
        throw err;
    }
}
 ...
 */
export async function appendReconcileLedgerEntry(input, opts = {}) {
 ...
 */
export async function removeReconcileLedgerEntry(entryId, opts = {}) {
    const ledger = await readReconcileLedger(opts);
    const filtered = ledger.entries.filter((e) => e.id !== entryId);
    if (filtered.length === ledger.entries.length)
 ...
 */
export function findReconcileLedgerEntriesFor(ledger, name, client, manifestPath) {
    return ledger.entries
        .filter((e) => e.name === name && e.client === client && e.manifestPath === manifestPath)
        .sort((a, b) => b.appliedAt.localeCompare(a.appliedAt));
}
//# sourceMappingURL=manifest-reconcile-ledger.js.map