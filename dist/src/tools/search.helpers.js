 ...
 */
import { CLIENT_TO_COMPATIBILITY_SLUG, deriveSecuritySummaryFromApiSkill, deriveSecuritySummaryFromSkillRow, } from '@skillsmith/core';
 ...
 */
export function resolveSearchLimit(limit) {
    const numeric = typeof limit === 'number'
        ? limit
 ...
 */
export function compatibilityWantedSlugs(filter) {
...
export function computeLocalCompatFetchLimit(limit, filterActive) {
    if (!filterActive)
        return limit;
 ...
 */
export function computeCompatRank(compatibility, wanted) {
    if (wanted.size === 0)
        return 1;
 ...
 */
export function sortByCompatRank(results, filter) {
 ...
 */
export function countCompatDeprioritized(results, filter) {
    const wanted = compatibilityWantedSlugs(filter);
    if (wanted.size === 0)
        return 0;
