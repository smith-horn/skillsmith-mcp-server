        ...
        client: {
            ...
            rpc: async (fn, params) => {
                ...
                if (fn === 'review_private_registry_submission') {
                    const record = { table: fn, op: 'select', filters: [], params };
                    calls.push(record);
                    const r = respond(record);
                    if (r.error)
...
describe('SMI-6202 — review(): RBAC seam widening via team_permission_grants', () => {
    ...
    const notAdminMessage = (teamId) => 'only a team admin or owner, or a holder of an explicit registry:approve grant, may ' +
        'review ' +
    ...
    it('b1: a plain member with no grant is refused, with the widened RAISE message', async () => {
        ...
        await expect(service.review(TEAM, SKILL, '1.0.0', 'approved')).rejects.toThrow(notAdminMessage(TEAM));
    });
    it('b2: a member holding an explicit registry:approve allow grant now succeeds (new capability)', async () => {
        const user = createRecorder((record) => record.table === 'review_private_registry_submission'
    ...
    it('cross-team: an admin/grant-holder on one team cannot review a submission for another team', async () => {
        ...
        const user = createRecorder((record) => {
            ...
            return {
                data: null,
                error: { code: '42501', message: notAdminMessage(record.params?.p_team_id) },
            };
        });
        const admin = createRecorder();
        await setClients(user.client, admin.client);
        const service = createLiveRegistryService();
        await expect(service.review(OTHER_TEAM, SKILL, '1.0.0', 'approved')).rejects.toThrow(notAdminMessage(OTHER_TEAM));
...
//# sourceMappingURL=registry-tools.live.review-rbac-widening.test.js.map