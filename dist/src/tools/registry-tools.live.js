...
import { mapSubmissionRow, readBackSubmission, listSubmissions, reviewSubmission, } from './registry-tools.live.submissions.js';
...
async function getClient() {
    try {
        ...
        throw new Error(`Private registry operations require SUPABASE_SERVICE_ROLE_KEY on the MCP host: ${message}`);
    }
}
/**
...
async function setDeprecated(teamId, skillId, value) {
    ...
    if (probe.error && !isNoRowsError(probe.error)) {
        ...
        throw new Error(`Failed to ${operation} skill: the update matched no rows and the follow-up check for ` +
    ...
    if (Array.isArray(probe.data) && probe.data.length > 0) {
        ...
        throw new Error(`Only team admins can ${operation} "${skillId}". Your account is a member of this team but ` +
...
function prepareContent(content) {
    ...
    if (bytes > MAX_CONTENT_BYTES) {
        throw new Error(`Skill content is ${bytes} bytes, over the ${MAX_CONTENT_BYTES}-byte (2 MB) private-registry limit. Split large assets out of the skill package.`);
    }
...
export function createLiveRegistryService() {
    return {
        async publish(teamId, skillId, version, content, description) {
            ...
            if (insertResp.error) {
                ...
                if (isUniqueViolation(insertResp.error)) {
                    throw new Error(`Version ${version} of "${skillId}" already exists in this team's private registry. ` +
        ...
        async review(teamId, skillId, version, decision, note) {
            const { client, actorUserId } = await getMemberUserClient(decision === 'approved' ? 'approve' : 'reject');
            return reviewSubmission({ client, teamId, skillId, version, decision, note, actorUserId });
        },
    };
}
//# sourceMappingURL=registry-tools.live.js.map