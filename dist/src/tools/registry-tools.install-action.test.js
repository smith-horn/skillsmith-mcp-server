 ...
 */
function makeInstallerAntigravity() {
    return new SkillInstallationService({
        db,
        skillRepo: new SkillRepository(db),
        skillDependencyRepo: new SkillDependencyRepository(db),
...
beforeEach(async () => {
    ...
    initializeSchema(db);
    tmpDir = path.join(os.tmpdir(), `skillsmith-registry-install-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('private_registry_manage(action:"install") — dispatch', () => {
    ...
    it('reports a never-published skill as not found, in action:"get"\'s exact words', async () => {
        ...
        const result = await executePrivateRegistryManage({ action: 'install', skillId: 'myteam/nope' }, mockContext);
        expect(result.success).toBe(false);
        expect(result.error).toBe('Skill "myteam/nope" not found in private registry. If you expect this to exist, check ' +
            'with a team admin.');
    });
});
// ---------------------------------------------------------------------------
...
describe('private_registry_manage(action:"install") — publish → install round-trip', () => {
    it('writes every published file to disk with private-registry provenance', async () => {
        ...
        const installedSkillMd = await fs.readFile(path.join(skillsDir, 'acme-tool', 'SKILL.md'), 'utf-8');
        expect(installedSkillMd).toContain(SECRET_LINE);
        const installedExtra = await fs.readFile(path.join(skillsDir, 'acme-tool', 'examples.md'), 'utf-8');
