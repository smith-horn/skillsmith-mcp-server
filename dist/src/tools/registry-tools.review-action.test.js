...
describe('private_registry_manage approve/reject actions — SMI-5949 D-5/D-6', () => {
    ...
    it('reject succeeds, calling the RPC with p_decision="rejected"', async () => {
        const { client, rpcCalls } = createFakeClient();
        await mockBothClients(client);
        const result = await executePrivateRegistryManage({ action: 'reject', skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
        expect(result.success).toBe(true);
    ...
    it('requires both skillId and version — neither the RPC nor a partial call is attempted', async () => {
        const { client, rpcCalls } = createFakeClient();
        await mockBothClients(client);
        const noSkillId = await executePrivateRegistryManage({ action: 'approve', version: '1.0.0' }, makeContext());
        const noVersion = await executePrivateRegistryManage({ action: 'reject', skillId: 'myteam/skill-a' }, makeContext());
    ...
    it.each([
        { action: 'approve', mustMatch: /approved/i },
        { action: 'reject', mustMatch: /rejected|terminal|resubmit/i },
    ])('$action message explicitly disclaims a full content read (finding C1)', async ({ action, mustMatch }) => {
        const { client } = createFakeClient();
        await mockBothClients(client);
...
//# sourceMappingURL=registry-tools.review-action.test.js.map