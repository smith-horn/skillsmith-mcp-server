...
import { executeRbacManage, executeRbacCreatePolicy, createStubRBACService, setRBACService, } from './rbac-tools.js';
import { META_PERMISSIONS, MANAGE_RBAC_PERMISSION } from './rbac-tools.types.js';
import { STUB_TEAM_ID } from './rbac-tools.stub.js';
import { isPermissionDeniedError, permissionErrorText, toPermissionDeniedError, permissionDeniedMessage, } from './team-permission-error.js';
const mockContext = {};
/** The exact sentence gate 4b raises, parameterised the same way the SQL's `%` is. */
const notGrantable = (permission) => `The "${permission}" permission is owner-only and cannot be granted to another role.`;
const GRANTABLE_ROLES_UNDER_TEST = ['admin', 'member'];
const ORDINARY_PERMISSIONS = ['registry:approve', 'registry:deprecate'];
describe('SMI-6319: meta-permissions are non-delegable', () => {
        ...
        for (const permission of META_PERMISSIONS) {
            for (const role of GRANTABLE_ROLES_UNDER_TEST) {
                it(`refuses set_role_permission(${role}, ${permission}, allow) — the SMI-6312 repro`, async () => {
                    const result = await executeRbacManage({ action: 'set_role_permission', role, permission, effect: 'allow' }, mockContext);
                    ...
                    expect(listed.permissions?.find((p) => p.role === role && p.permission === permission)).toMatchObject({ effect: 'deny', source: 'default' });
                });
            }
        }
    ...
    describe('every other grant is completely unaffected', () => {
            ...
            for (const permission of ORDINARY_PERMISSIONS) {
                for (const effect of ['allow', 'deny']) {
                    it(`still writes ${effect} on ${permission} for ${role}`, async () => {
                        const result = await executeRbacManage({ action: 'set_role_permission', role, permission, effect }, mockContext);
                        expect(result.success).toBe(true);
                        const listed = await executeRbacManage({ action: 'list_roles' }, mockContext);
                        expect(listed.permissions?.find((p) => p.role === role && p.permission === permission)).toMatchObject({ effect, source: 'grant' });
                    });
                }
            }
        }
        it('still clears an ordinary override with reset_role_permission', async () => {
            ...
            const cleared = await executeRbacManage({ action: 'reset_role_permission', role: 'member', permission: 'registry:approve' }, mockContext);
        ...
        for (const permission of META_PERMISSIONS) {
            it(`still allows the OWNER to write a DENY row for ${permission}`, async () => {
                const result = await executeRbacManage({ action: 'set_role_permission', role: 'admin', permission, effect: 'deny' }, mockContext);
                expect(result.success).toBe(true);
                const listed = await executeRbacManage({ action: 'list_roles' }, mockContext);
                expect(listed.permissions?.find((p) => p.role === 'admin' && p.permission === permission)).toMatchObject({ effect: 'deny', source: 'grant' });
            });
            it(`still allows the OWNER to clear a ${permission} deny row`, async () => {
                await executeRbacManage({ action: 'set_role_permission', role: 'admin', permission, effect: 'deny' }, mockContext);
                const cleared = await executeRbacManage({ action: 'reset_role_permission', role: 'admin', permission }, mockContext);
                expect(cleared.success).toBe(true);
            });
            ...
            it(`refuses upgrading an existing ${permission} deny row to allow`, async () => {
                await executeRbacManage({ action: 'set_role_permission', role: 'admin', permission, effect: 'deny' }, mockContext);
                const upgrade = await executeRbacManage({ action: 'set_role_permission', role: 'admin', permission, effect: 'allow' }, mockContext);
                expect(upgrade.success).toBe(false);
                expect(permissionErrorText(upgrade.error)).toBe(notGrantable(permission));
                const listed = await executeRbacManage({ action: 'list_roles' }, mockContext);
                expect(listed.permissions?.find((p) => p.role === 'admin' && p.permission === permission)).toMatchObject({ effect: 'deny' });
            });
        }
    });
    // ==========================================================================
    ...
    describe('rbac_create_policy (the bulk/wildcard writer) is covered too', () => {
        ...
        it('a refused bulk expansion writes no grant rows at all', async () => {
            ...
            const listed = await executeRbacManage({ action: 'list_roles' }, mockContext);
            const written = listed.permissions?.filter((p) => META_PERMISSIONS.includes(p.permission) && p.source === 'grant');
        ...
        it('the bulk DELETE path still clears an ordinary registry policy', async () => {
            ...
            const removed = await executeRbacCreatePolicy({ action: 'delete', role: 'member', resources: ['registry'], actions: ['approve'] }, mockContext);
    ...
    describe('the live 42501 renders as authored copy, not the generic sentence', () => {
        ...
        for (const permission of META_PERMISSIONS) {
            it(`maps gate 4b's ${permission} refusal to its own authored text`, () => {
                const mapped = toPermissionDeniedError({ code: '42501', message: notGrantable(permission) }, MANAGE_RBAC_PERMISSION);
        ...
        it('still falls back to the generic sentence for an unrecognised 42501', () => {
            ...
            const mapped = toPermissionDeniedError({ code: '42501', message: 'permission denied for table team_permission_grants' }, MANAGE_RBAC_PERMISSION);
        ...
        it('does not classify the table CHECK violation (23514) as a permission denial', () => {
            ...
            expect(toPermissionDeniedError({
                code: '23514',
                message: 'new row for relation "team_permission_grants" violates check constraint ' +
                    '"team_permission_grants_meta_permission_not_grantable"',
            }, MANAGE_RBAC_PERMISSION)).toBeNull();
        });
    });
});
//# sourceMappingURL=rbac-tools.meta-permission-not-grantable.test.js.map