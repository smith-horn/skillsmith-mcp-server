...
export function validateMetadata(metadata, strict) {
    ...
    if (metadata.compatibility === undefined && (metadata.version !== undefined || strict)) {
        errors.push({
            field: 'compatibility',
            message: 'Field "compatibility" is recommended for published skills (e.g. ["claude-code", "cursor", "claude"]). ' +
 ...
 */
export function detectClaudeMdModification(body) {
    const warnings = [];
    ...
    if (modifiesClaudeMd) {
        warnings.push('This skill appears to modify CLAUDE.md. ' +
 ...
 */
export function validateNameMatchesDirectory(name, skillPath, isDirectory) {
    ...
    if (name !== actualDirName) {
        return [
            {
                field: 'name',
                message: `Frontmatter "name" ("${name}") does not match the skill's directory name ` +
