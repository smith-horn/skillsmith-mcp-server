...
import { DEFAULT_ROLE_PERMISSIONS, GRANTABLE_ROLES, MANAGE_RBAC_PERMISSION, META_PERMISSIONS, TEAM_PERMISSIONS, resolveEffectivePermission, } from './rbac-tools.types.js';
/**
...
const DEFAULT_STUB_ACTOR = {
    userId: 'stub-owner',
    teamId: STUB_TEAM_ID,
    role: 'owner',
};
const DEFAULT_STUB_MEMBERS = [
    {
...
export function createStubRBACService() {
    ...
    let actor = { ...DEFAULT_STUB_ACTOR };
    let members = DEFAULT_STUB_MEMBERS.map((m) => ({ ...m }));
    const grantKey = (teamId, role, permission) => `${teamId}::${role}::${permission}`;
    /** `has_team_permission(p_team_id, p_permission)` in miniature. Never returns undefined. */
    const hasPermission = (teamId, permission) => {
    ...
    const requirePermission = (teamId, permission) => {
        if (!hasPermission(teamId, permission))
            throw new TeamPermissionDeniedError(permission);
    };
    /**
     ...
     */
    const requireGrantWriteAuthority = (role, permission, effect) => {
        if (META_PERMISSIONS.includes(permission) && actor.role !== 'owner') {
            throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION, `Only the team owner can change who holds the "${permission}" permission.`);
        }
        ...
        if (META_PERMISSIONS.includes(permission) && effect === 'allow') {
            throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION, `The "${permission}" permission is owner-only and cannot be granted to another role.`);
        }
        const wouldWiden = effect === 'allow' ||
            (effect === 'reset' && DEFAULT_ROLE_PERMISSIONS[role][permission] === 'allow');
        if (wouldWiden && actor.role !== 'owner' && actor.role !== 'admin') {
            throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION, "Only owners and admins can widen a role's permissions. You can review permissions and " +
    ...
    return markAsStub({
        async listPermissions(teamId) {
            ...
            return GRANTABLE_ROLES.flatMap((role) => TEAM_PERMISSIONS.map((permission) => resolveEffectivePermission(role, permission, grants.get(grantKey(teamId, role, permission)))));
        },
        ...
        async setMemberRole(memberId, role) {
            ...
            if (!target)
                throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION);
            requirePermission(actor.teamId, MANAGE_RBAC_PERMISSION);
            if (target.role === 'owner') {
                throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION, "cannot change the team owner's role");
            }
            if (target.role === 'admin' && actor.role !== 'owner') {
                throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION, "forbidden: only the team owner can change an admin's role");
            }
            if (role === 'admin' && actor.role !== 'owner' && actor.role !== 'admin') {
                throw new TeamPermissionDeniedError(MANAGE_RBAC_PERMISSION, 'forbidden: only owners and admins can promote a member to admin');
            }
            target.role = role;
        },
        setActor(next) {
...
//# sourceMappingURL=rbac-tools.stub.js.map