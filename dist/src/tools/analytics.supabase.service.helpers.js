 ...
 */
export function nicknameFromActor(actorHashHex) {
 ...
 */
export function actorDisplayLabel(actor) {
...
export function buildCoverageNote(coverage) {
    ...
    const QUALITATIVE_SENTENCE = 'Coverage is shown only when at least five seats are reporting and at least five are not.';
    if (coverage === null) {
        ...
        if (coverageLevel === 'full' && canRenderRatio && canRenderSplit) {
            parts.push(`Reporting coverage: ${reportingSeats} of ${totalSeats} seats reporting ` +
    ...
    parts.push('These counts are self-reported by each client and are not independently verified — a usage ' +
        'view, not an audit record.');
    return parts.join(' ');
}
//# sourceMappingURL=analytics.supabase.service.helpers.js.map