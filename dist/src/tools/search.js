...
// circular dependency).
import { mergeRankAndPage, compatibilityWantedSlugs, computeLocalCompatFetchLimit, mapApiSkillToSearchResult, mapLocalSkillToSearchResult, resolveDefaultCompatibility, buildEmptySearchSuggestion, resolveSearchLimit, } from './search.helpers.js';
...
async function executeSearchImpl(input, context) {
    ...
    if (!context.apiClient.isOffline()) {
        try {
            ...
            const { pageResults, mergedTotal, discoveryOnlyHidden, compatibilityDeprioritized } = mergeRankAndPage({
                localResults,
                apiResults: results,
