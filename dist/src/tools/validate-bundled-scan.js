...
import { BUNDLED_SCAN_FILES, classifyBundledFile, extractPackageJsonLifecycleScripts, isRejectableScan, } from '@skillsmith/core/services/skill-installation-policy';
/**
 ...
 */
export async function scanBundledSiblings(skillPath, riskThreshold = 40) {
    const errors = [];
    ...
    for (const siblingFile of BUNDLED_SCAN_FILES) {
        ...
        if (fileClass === 'doc' || fileClass === 'config')
            continue;
        let siblingContent;
        try {
        ...
        const topFinding = report.findings.find((f) => f.severity === 'critical' || f.severity === 'high') ??
            report.findings.find((f) => f.type === 'code_execution' || f.type === 'obfuscated_directive') ??
            report.findings[0];
        const matchedLabel = topFinding ? (topFinding.category ?? topFinding.type) : 'unknown';
        errors.push({
            field: siblingFile,
            message: `Security scan failed for "${siblingFile}": ${report.findings.length} finding(s) ` +
...
//# sourceMappingURL=validate-bundled-scan.js.map