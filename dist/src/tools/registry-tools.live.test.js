...
import { executePrivateRegistryPublish, executePrivateRegistryManage, setPrivateRegistryService, createStubRegistryService, } from './registry-tools.js';
 ...
 */
async function mockBothClients(client) {
...
describe('private_registry_publish live mode — SMI-5816', () => {
    it('inserts with the resolved team_id and a SKILL.md-derived content_hash', async () => {
        ...
        vi.mocked(getSupabaseAdminClient).mockResolvedValue(client);
        const result = await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: SAMPLE_CONTENT }, makeContext());
    ...
    it('rejects content over the 2 MB cap before hitting the database', async () => {
        ...
        const result = await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: huge }, makeContext());
        expect(result.success).toBe(false);
    ...
    it('rejects content missing a SKILL.md entry', async () => {
        ...
        const { getSupabaseAdminClient } = await import('../supabase-client.js');
        vi.mocked(getSupabaseAdminClient).mockResolvedValue(client);
        const result = await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: { 'other.txt': 'x' } }, makeContext());
        expect(result.success).toBe(false);
    ...
    it('surfaces a typed error when SUPABASE_SERVICE_ROLE_KEY is not configured', async () => {
        ...
        vi.mocked(getSupabaseAdminClient).mockRejectedValue(new Error('Supabase admin not configured: SUPABASE_SERVICE_ROLE_KEY required'));
        const result = await executePrivateRegistryPublish({ skillId: 'myteam/skill-a', version: '1.0.0', content: SAMPLE_CONTENT }, makeContext());
        expect(result.success).toBe(false);
...
describe('private_registry_publish namespace pre-check — SMI-5852', () => {
    it('rejects a mismatched skill_id namespace before ever attempting an insert', async () => {
        ...
        const { getSupabaseAdminClient } = await import('../supabase-client.js');
        vi.mocked(getSupabaseAdminClient).mockResolvedValue(client);
        const result = await executePrivateRegistryPublish({ skillId: 'anthropic/commit', version: '1.0.0', content: SAMPLE_CONTENT }, makeContext());
        expect(result.success).toBe(false);
...
describe('private_registry_manage live mode — team scoping — SMI-5816', () => {
    ...
    it('get filters by resolved team_id + skill_id + version', async () => {
        ...
        const { getSupabaseAdminClient } = await import('../supabase-client.js');
        vi.mocked(getSupabaseAdminClient).mockResolvedValue(client);
        const result = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
        expect(result.success).toBe(true);
        const q = calls.find((c) => c.table === 'private_registry_skills');
        expect(q.filters.some((f) => f.column === 'team_id' && f.value === RESOLVED_TEAM)).toBe(true);
        expect(q.filters.some((f) => f.column === 'skill_id' && f.value === 'myteam/skill-a')).toBe(true);
    ...
    it('deprecate updates deprecated=true scoped to team_id + skill_id', async () => {
        ...
        const result = await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/skill-a' }, makeContext());
        expect(result.success).toBe(true);
    ...
    it('deprecate returns not-found when no row in this team matches', async () => {
        ...
        await mockBothClients(client);
        const result = await executePrivateRegistryManage({ action: 'deprecate', skillId: 'myteam/ghost' }, makeContext());
        expect(result.success).toBe(false);
    ...
    it.each([
        { action: 'deprecate', deprecated: true },
        { action: 'undeprecate', deprecated: false },
        ...
        await mockBothClients(client);
        const result = await executePrivateRegistryManage({ action: c.action, skillId: 'myteam/skill-a' }, makeContext());
        expect(result.success).toBe(true);
    });
    ...
    it('get returns null (not-found) for PostgREST’s genuine no-rows code', async () => {
        ...
        const { getSupabaseAdminClient } = await import('../supabase-client.js');
        vi.mocked(getSupabaseAdminClient).mockResolvedValue(client);
        const result = await executePrivateRegistryManage({ action: 'get', skillId: 'myteam/ghost', version: '1.0.0' }, makeContext());
        expect(result.success).toBe(false);
        expect(result.error).toMatch(/not found/i);
    });
});
//# sourceMappingURL=registry-tools.live.test.js.map