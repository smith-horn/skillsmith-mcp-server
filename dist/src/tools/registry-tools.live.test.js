...
import { executePrivateRegistryPublish, setPrivateRegistryService, createStubRegistryService, } from './registry-tools.js';
import { createLiveRegistryService } from './registry-tools.live.js';
import { RESOLVED_TEAM, SAMPLE_CONTENT, createFakeClient, makeContext, publishedRow, } from './registry-tools.live.test-helpers.js';
...
describe('private_registry_publish live mode — SMI-5816', () => {
    it('inserts on the USER client (not service-role) with the resolved team_id, and sends no content_hash — SMI-5949 D-7/D-4(a)', async () => {
        ...
        const { getSupabaseAdminClient, getSupabaseUserClient, getSupabaseClient } = await import('../supabase-client.js');
    ...
    it('rejects content over the 2 MB cap before hitting the database', async () => {
        ...
        expect(calls.find((c) => c.op === 'insert' && c.table === 'private_registry_skills')).toBeUndefined();
    });
