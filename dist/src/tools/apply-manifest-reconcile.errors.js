 ...
 */
export function describeReconcileError(code, ctx = {}) {
    switch (code) {
        ...
        case 'manifest.reconcile.entry_not_found':
            return (`No manifest entry found for '${ctx.name ?? '<unknown>'}'` +
        ...
        case 'manifest.reconcile.key_shape_ambiguous':
            return (`Ambiguous manifest key for '${ctx.name ?? '<unknown>'}': found both '${ctx.manifestKey ?? '<unknown>'}' ` +
        ...
        case 'manifest.reconcile.relink_unvalidated':
            return (`Could not confirm registry id '${ctx.id ?? '<unknown>'}' exists in the registry` +
        ...
        case 'manifest.reconcile.drop_target_still_resolves':
            return (`Refusing to drop '${ctx.name ?? '<unknown>'}': its installPath` +
                (ctx.path ? ` ('${ctx.path}')` : '') +
        ...
        case 'manifest.reconcile.revert_ambiguous':
            return (`revert is ambiguous for '${ctx.name ?? '<unknown>'}': ${(ctx.candidateIds ?? []).length} ledger entries match ` +
        ...
        case 'manifest.reconcile.ledger_version_unsupported':
            return (`manifest-reconcile-ledger.json version ${String(ctx.found)} is newer than supported version ${String(ctx.expected)}. ` +
        ...
        case 'manifest.reconcile.verify_unavailable':
            return (`Could not reach the registry to verify '${ctx.name ?? '<unknown>'}'${ctx.detail ? ` (${ctx.detail})` : '.'} ` +
...
//# sourceMappingURL=apply-manifest-reconcile.errors.js.map