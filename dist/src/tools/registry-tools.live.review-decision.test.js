...
import { executePrivateRegistryManage, setPrivateRegistryService, createStubRegistryService, } from './registry-tools.js';
import { createLiveRegistryService } from './registry-tools.live.js';
import { RESOLVED_TEAM, createFakeClient, makeContext, mockBothClients, } from './registry-tools.live.test-helpers.js';
...
describe('review_private_registry_submission RPC errors — verbatim passthrough (M10)', () => {
    it.each([
        {
            ...
            error: {
                ...
                message: 'only a team admin or owner, or a holder of an explicit registry:approve grant, may ' +
                    'review ' +
        ...
        },
        {
            ...
            error: {
                code: 'P0001',
                message: 'This submission has already been approved and cannot be reviewed again — approved ' +
        ...
        },
        {
            ...
            error: {
                code: '23514',
                message: 'This submission has no recorded submitter (published_by is NULL) and cannot be ' +
        ...
        const result = await executePrivateRegistryManage({ action, skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
    ...
    it('does not remap a SQLSTATE to any canned message', async () => {
        const { client } = createFakeClient(reviewRpcError({ code: '42501', message: 'Only team admins can review submissions.' }));
        await mockBothClients(client);
        const result = await executePrivateRegistryManage({ action: 'approve', skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
...
describe('approve/reject audit rows — SMI-5949 Wave 2 Step 4', () => {
    it('writes a success audit row with the correct event_type and actor', async () => {
        ...
        await mockBothClients(client);
        await executePrivateRegistryManage({ action: 'approve', skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
    ...
    it('writes a "reject" audit row distinctly from "approve"', async () => {
        const { client, calls } = createFakeClient();
        await mockBothClients(client);
        await executePrivateRegistryManage({ action: 'reject', skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
        const audit = calls.find((c) => c.table === 'audit_logs' && c.op === 'insert');
    ...
    it('writes a denied audit row (not "error") for every documented RPC rejection', async () => {
        const { client, calls } = createFakeClient(reviewRpcError({ code: '42501', message: 'Only team admins can review submissions.' }));
        await mockBothClients(client);
        await executePrivateRegistryManage({ action: 'approve', skillId: 'myteam/skill-a', version: '1.0.0' }, makeContext());
        const audit = calls.find((c) => c.table === 'audit_logs' && c.op === 'insert');
        expect(audit).toBeDefined();
        expect(audit.payload?.result).toBe('denied');
        expect((audit.payload?.metadata).detail).toBe('42501');
    });
    it('audit rows carry team_id/skill_id/version scoped to this operation', async () => {
        const { client, calls } = createFakeClient();
        await mockBothClients(client);
        await executePrivateRegistryManage({ action: 'approve', skillId: 'myteam/skill-a', version: '2.1.0' }, makeContext());
        const audit = calls.find((c) => c.table === 'audit_logs' && c.op === 'insert');
...
//# sourceMappingURL=registry-tools.live.review-decision.test.js.map