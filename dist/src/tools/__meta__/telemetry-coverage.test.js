 ...
 */
async function listToolSourceFiles() {
...
describe('SMI-5018: MCP tool telemetry coverage (v1, MCP tree only)', () => {
    ...
    it('every listed dispatcher export is telemetry-wrapped', async () => {
        ...
        if (failures.length > 0) {
            throw new Error(`Telemetry coverage failures (${failures.length}):\n` +
    ...
    it('all files in DISPATCHER_MAP exist on disk', async () => {
        ...
        if (missingFromDisk.length > 0) {
            throw new Error(`DISPATCHER_MAP references files that do not exist on disk:\n` +
    ...
    it('reports dispatcher coverage to CI output', () => {
        ...
        console.info(`[SMI-5018] MCP telemetry coverage: ${dispatcherCount} dispatchers across ${fileCount} files.`);
