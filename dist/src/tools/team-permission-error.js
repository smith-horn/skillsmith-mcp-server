...
export function permissionDeniedMessage(permission) {
    return (`You don't have the "${permission}" permission for this team. ` +
...
function readPostgresError(err) {
    if (typeof err !== 'object' || err === null)
        return null;
    const candidate = err;
    const code = typeof candidate.code === 'string' ? candidate.code : null;
    const message = typeof candidate.message === 'string' ? candidate.message : '';
    if (code === null && message.length === 0)
        return null;
    return { code, message };
}
/**
 ...
 */
export function toPermissionDeniedError(err, requiredPermission) {
    if (err instanceof TeamPermissionDeniedError)
        return err.toErrorShape();
    const pg = readPostgresError(err);
    if (!pg)
        return null;
    const text = pg.message.trim();
    const isDenial = pg.code === POSTGRES_INSUFFICIENT_PRIVILEGE || text.toLowerCase() === PERMISSION_DENIED_CODE;
    if (!isDenial)
        return null;
 ...
 */
export function permissionErrorText(error) {
    if (error === undefined)
        return '';
    return typeof error === 'string' ? error : error.message;
}
//# sourceMappingURL=team-permission-error.js.map