 ...
 */
export function mapSubmissionRow(teamId, row) {
    return {
        ...
        registryUrl: row.approval_status === 'approved'
 ...
 */
export async function readBackSubmission(client, teamId, skillId, version) {
    const resp = await client.rpc('get_private_registry_submissions', { p_team_id: teamId, p_status: 'pending' });
    if (resp.error) {
        throw new Error(`Skill ${skillId}@${version} was published, but the confirmation read-back failed: ` +
            `${resp.error.message ?? 'unknown error'}`);
    }
    const match = (resp.data ?? []).find((row) => row.skill_id === skillId && row.version === version);
    if (!match) {
        throw new Error(`Skill ${skillId}@${version} was published but could not be confirmed by read-back. It ` +
 ...
 */
export async function listSubmissions(client, teamId, status) {
    const resp = await client.rpc('get_private_registry_submissions', {
        p_team_id: teamId,
...
export async function reviewSubmission(params) {
    ...
    const resp = await client.rpc('review_private_registry_submission', {
        p_team_id: teamId,
        p_skill_id: skillId,
        p_version: version,
        p_decision: decision,
    ...
    if (!row) {
        ...
        throw new Error(`${operation === 'approve' ? 'Approval' : 'Rejection'} of ${skillId}@${version} reported ` +
...
//# sourceMappingURL=registry-tools.live.submissions.js.map