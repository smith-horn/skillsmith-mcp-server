...
import { SupabaseAnalyticsService, buildCoverageNote, actorDisplayLabel, } from './analytics.supabase.service.js';
 ...
 */
export async function resolveCloudAnalytics() {
    const accessToken = await resolveUserAccessToken();
    if (!accessToken) {
        return {
            ok: false,
            error: 'Not signed in. Run `skillsmith login` to view Team/Enterprise usage analytics — ' +
    ...
    if (!teamId) {
        return {
            ok: false,
            error: 'Unable to resolve your team. Set SKILLSMITH_LICENSE_KEY or SKILLSMITH_API_KEY to an ' +
...
// ============================================================================
const SKILL_PANEL_HONESTY_LINE = 'Skill-invocation data: not yet captured for teams (Claude Code hook invocations are not ' +
    'team-attributed).';
function sign(n) {
    return n >= 0 ? '+' : '';
}
/** AC-10 read-side: an actionable error IS the tool's entire output — heading + error text. */
function renderCloudError(title, error) {
    return [title, '', '## Error', error].join('\n');
}
function csvEscape(s) {
    return s.replace(/"/g, '""');
}
function topToolsTableWithPct(topTools, total) {
    return [
        '| Tool | Calls | % of Total |',
        '|------|-------|------------|',
        ...topTools.map((t) => {
...
function topToolsTableSimple(topTools) {
    return [
        '| Tool | Calls |',
        '|------|-------|',
...
function dailyTrendTable(dailyTrend) {
    return [
        '| Date | Calls |',
        '|------|-------|',
...
function renderDataCoverageLines(coverageResult) {
 ...
 */
async function executeTeamAnalyticsDashboardImpl(input, _context) {
    ...
    if (!resolved.ok)
        return renderCloudError(title, resolved.error);
    ...
    if (!usage.ok)
        return renderCloudError(title, usage.error);
 ...
 */
async function executeTeamUsageReportImpl(input, _context) {
    const days = periodDays(input.period);
    ...
    lines.push(...renderDataCoverageLines(coverageResult), ...renderEmitHealthLines(getTelemetryEmitStats()));
    return lines.join('\n');
}
/**
 ...
 * actually populate.
 */
async function executeAnalyticsDashboardImpl(input, _context) {
    const days = periodDays(input.period);
    ...
    if (input.includeRecommendations) {
        lines.push('', '## Recommendation Accuracy', '_Recommendation accuracy tracking is not yet implemented for cloud-backed analytics — ' +
 ...
 */
async function executeUsageReportImpl(input, _context) {
    const days = periodDays(input.period);
...
//# sourceMappingURL=analytics.actions.js.map