...
import { SsoAuthError, SsoValidationError, SsoDomainNotVerifiedError, SsoDomainClaimedByAnotherTeamError, SsoDomainVerificationFailedError, SsoDomainNotClaimedError, SsoExpireUnavailableError, SsoServiceUnavailableError, } from './sso-tools.live.errors.js';
...
function functionsBaseUrl() {
    ...
    if (!url) {
        throw new Error('SUPABASE_URL is not configured — createLiveSSOService() must only run when ' +
...
function supabaseAnonKey() {
    ...
    if (!key) {
        throw new Error('SUPABASE_ANON_KEY is not configured — createLiveSSOService() must only run when ' +
            'isSupabaseConfigured() is true.');
    }
    return key;
}
 ...
 */
async function throwMappedError(res) {
    ...
    switch (res.status) {
        ...
        case 409:
            ...
            if (body?.error === 'domain_verified_by_another_team') {
                throw new SsoDomainClaimedByAnotherTeamError(body.message ?? 'This domain is already verified by another team.');
            }
            if (body?.error === 'domain_verification_failed') {
                throw new SsoDomainVerificationFailedError(body.message ?? 'The domain could not be verified. Confirm the TXT record and try again.');
            }
        ...
        case 404:
            if (body?.error === 'domain_not_claimed') {
                throw new SsoDomainNotClaimedError(body.message ?? 'No pending claim for this domain — call claim_domain first.');
            }
            throw new SsoServiceUnavailableError(res.status);
        case 501:
            throw new SsoExpireUnavailableError(body?.message ??
...
async function fetchSsoManage(binding, payload) {
    try {
...
async function callSsoManage(binding, payload) {
    const res = await fetchSsoManage(binding, payload);
    if (!res.ok)
        await throwMappedError(res);
    const body = await readJson(res);
    if (body === null) {
 ...
 * one.
 */
function mapProviderToConfig(row) {
    return {
        protocol: 'saml',
 ...
 */
function mapSettingsToConfig(body) {
    if (!body.settings)
        return null;
    return {
        protocol: 'saml',
 ...
 */
export function createLiveSSOService() {
    return {
        async set(config) {
            ...
            if (config.protocol !== 'saml') {
                throw new SsoValidationError('Only the "saml" protocol is available today — the live SSO service wraps GoTrue\'s ' +
            ...
            const body = await callSsoManage(binding, {
                action: 'set',
        ...
        async remove(memberDisposition) {
        ...
        async claimDomain(domain) {
            ...
            // builds from them.
            const body = await callSsoManage(binding, { action: 'claim_domain', domain });
            return {
                domain: body.domain,
...
//# sourceMappingURL=sso-tools.live.js.map