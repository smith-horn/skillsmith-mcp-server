...
export const privateRegistryPublishInputSchema = z.object({
    ...
    version: z
        .string()
        .regex(/^\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?(?:\+[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?$/, 'Must be a valid semver version')
        .describe('Semver version to publish'),
    content: skillContentSchema.describe('Packaged skill files as a { path: text } map; must include a "SKILL.md" entry (max 2 MB total)'),
...
export const privateRegistryManageInputSchema = z.object({
    ...
    includeDeprecated: z
        .boolean()
        .optional()
        .describe('Action "list" only: include deprecated versions (omit or false to hide them, the ' +
...
export const privateRegistryPublishToolSchema = {
    ...
    description: "Publish a skill to your organization's private registry. " +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            content: {
                ...
                description: 'Packaged skill files as a { path: text } map; must include "SKILL.md" (max 2 MB total)',
            },
            description: {
                type: 'string',
    ...
    inputSchema: {
        ...
        properties: {
            action: {
                ...
                description: 'Registry operation to perform. "namespace" returns your team\'s registry ' +
            ...
            skillId: {
                type: 'string',
                description: 'Skill ID in author/name format (get/deprecate/undeprecate/install/approve/reject)',
            },
            version: {
                type: 'string',
                description: 'Version filter; required for approve/reject; "install" defaults to the most ' +
                    'recently published',
            },
            force: { type: 'boolean', description: 'Reinstall over an existing install' },
            includeDeprecated: {
                type: 'boolean',
                description: 'Action "list" only: include deprecated versions (default hides them). No effect on ' +
...
//# sourceMappingURL=registry-tools.schemas.js.map