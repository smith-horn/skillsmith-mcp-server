 ...
 */
import { z } from 'zod';
import { CLIENT_IDS, InvalidScopeValueError, UnsatisfiableWorkspaceScopeError, } from '@skillsmith/core/install';
import { withTelemetry } from '@skillsmith/core/telemetry';
import { getToolContext } from '../context.js';
import { runDropEntry, runMarkLocal, runRelink, runRevert, runVerify, } from './apply-manifest-reconcile.actions.js';
    ...
    inputSchema: {
        ...
        properties: {
            action: {
                ...
                description: "'mark_local' | 'relink' | 'drop_entry' | 'verify' | 'revert' — see tool description for each action's effect.",
            },
            name: {
                type: 'string',
                description: 'Skill name (manifest entry, not a full id). Required for mark_local/relink/drop_entry. Optional for verify (omit to batch-verify every entry) and revert (omit when passing ledgerEntryId).',
            },
            client: {
                type: 'string',
            ...
            scope: {
                ...
                description: 'Explicit install scope (ADR-139). Defaults to auto-detecting an EXISTING workspace directory, else global.',
            },
            cwd: {
                type: 'string',
                description: "Absolute path to the calling client's actual project root, used as the ancestor-walk starting point for workspace scope resolution.",
            },
            id: {
                type: 'string',
                description: "relink only, required together with 'source': registry skill id (author/name).",
            },
            source: {
                type: 'string',
                description: "relink only, required together with 'id': the source reference to record (e.g. github:owner/repo).",
            },
            reason: {
                type: 'string',
            ...
            ledgerEntryId: {
                type: 'string',
                description: 'revert only: precise ledger entry id (mrc_...) from a prior mutating action response. Preferred over name when known.',
            },
        },
        required: ['action'],
    },
};
async function applyManifestReconcileImpl(input, _context) {
    ...
    if (!parsed.success) {
        ...
        return {
            success: false,
            action: input?.action ?? 'mark_local',
    ...
    try {
        switch (validInput.action) {
            ...
            default: {
                const exhaustive = validInput.action;
...
//# sourceMappingURL=apply-manifest-reconcile.js.map