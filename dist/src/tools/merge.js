 ...
 */
function getHunks(base, modified, lcs) {
    const hunks = [];
    let baseIdx = 0;
    let modIdx = 0;
    let lcsIdx = 0;
 ...
 */
export function computeDiff(base, target) {
 ...
 */
export function threeWayMerge(base, local, upstream) {
    ...
    const localModified = new Map();
    const upstreamModified = new Map();
    ...
    const mergedLines = [];
    const conflicts = [];
    const processedLocalHunks = new Set();
    const processedUpstreamHunks = new Set();
    let baseIdx = 0;
    while (baseIdx <= baseLines.length) {
        ...
        if (localHunk && !processedLocalHunks.has(localHunk)) {
            ...
            if (upstreamHunk && !processedUpstreamHunks.has(upstreamHunk)) {
                ...
                baseIdx = Math.max(localHunk.baseStart + localHunk.baseCount, upstreamHunk.baseStart + upstreamHunk.baseCount);
                continue;
            }
            else {
