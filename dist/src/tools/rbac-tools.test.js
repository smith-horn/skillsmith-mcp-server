...
import { rbacManageInputSchema, rbacAssignRoleInputSchema, rbacCreatePolicyInputSchema, executeRbacManage, executeRbacAssignRole, executeRbacCreatePolicy, createStubRBACService, setRBACService, DEFAULT_ROLE_PERMISSIONS, } from './rbac-tools.js';
...
describe('rbac-tools', () => {
    ...
    describe('rbacManageInputSchema', () => {
        ...
        it('rejects an invalid permission', () => {
            expect(() => rbacManageInputSchema.parse({
                action: 'set_role_permission',
                role: 'admin',
    ...
    describe('rbacAssignRoleInputSchema', () => {
        ...
        it('rejects role="owner" (never assignable through this schema)', () => {
            expect(() => rbacAssignRoleInputSchema.parse({ action: 'assign', memberId: 'tm_1', role: 'owner' })).toThrow();
        });
    });
    ...
    describe('executeRbacManage: list_roles / get_role', () => {
        it('SMI-6242: list_roles reflects the corrected default matrix', async () => {
            ...
            const effectFor = (perms, p) => perms.find((x) => x.permission === p);
    ...
    describe('executeRbacManage: set_role_permission / reset_role_permission', () => {
        ...
        it('reset_role_permission reports false when there was nothing to clear', async () => {
            const result = await executeRbacManage({ action: 'reset_role_permission', role: 'admin', permission: 'registry:approve' }, mockContext);
            expect(result.success).toBe(true);
    ...
    describe('executeRbacManage: gate 3 (no team:manage_rbac) and gate 4/5 (owner-anchored writes)', () => {
        ...
        it('SMI-6319: the owner cannot elevate `admin` with team:manage_rbac (was gate 4, now unreachable)', async () => {
            ...
            expect(permissionErrorText(elevate.error)).toBe('The "team:manage_rbac" permission is owner-only and cannot be granted to another role.');
            const after = await executeRbacManage({ action: 'get_role', role: 'admin' }, mockContext);
            expect(after.role.permissions.find((p) => p.permission === 'team:manage_rbac')).toMatchObject({ effect: 'deny', source: 'default' });
        });
        ...
        it('SMI-6319: the owner cannot elevate `member` with team:manage_sso either (was gate 4 scope, now unreachable)', async () => {
            ...
            expect(permissionErrorText(elevate.error)).toBe('The "team:manage_sso" permission is owner-only and cannot be granted to another role.');
        });
        ...
        it('gate 4 scope (SMI-6319): the OWNER cannot write an allow to team:manage_sso either', async () => {
            ...
            expect(permissionErrorText(grant.error)).toBe('The "team:manage_sso" permission is owner-only and cannot be granted to another role.');
        });
        it('gate 4 scope: the OWNER can still write a deny and clear team:manage_sso', async () => {
            ...
            const cleared = await executeRbacManage({ action: 'reset_role_permission', role: 'admin', permission: 'team:manage_sso' }, mockContext);
        ...
        it('SMI-6319: reset on a meta-permission needs no reset-side twin of rule 1b -- the default is always deny', async () => {
            const result = await executeRbacManage({ action: 'reset_role_permission', role: 'admin', permission: 'team:manage_rbac' }, mockContext);
            expect(result.success).toBe(true);
            expect(result.message).toContain('already at the built-in default');
        });
    });
    ...
    describe('executeRbacAssignRole', () => {
        ...
        it('owner assigns admin to the default member', async () => {
            const result = await executeRbacAssignRole({ action: 'assign', memberId: 'tm_stub_member', role: 'admin' }, mockContext);
            expect(result.success).toBe(true);
            expect(result.message).toContain('now `admin`');
        });
        it('revoke rejects role="member" (removal is a separate operation)', async () => {
            const result = await executeRbacAssignRole({ action: 'revoke', memberId: 'tm_stub_admin', role: 'member' }, mockContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain('Only "admin" can be revoked');
        });
        it('revoke demotes an admin back to member', async () => {
            const result = await executeRbacAssignRole({ action: 'revoke', memberId: 'tm_stub_admin', role: 'admin' }, mockContext);
            expect(result.success).toBe(true);
            expect(result.message).toContain('now `member`');
        });
        it("owner protection: the owner's own role can never be changed", async () => {
            const result = await executeRbacAssignRole({ action: 'assign', memberId: 'tm_stub_owner', role: 'member' }, mockContext);
            expect(result.success).toBe(false);
        ...
        it('not-found member id is refused the same way as no-permission (no existence oracle)', async () => {
            const result = await executeRbacAssignRole({ action: 'assign', memberId: 'tm_does_not_exist', role: 'admin' }, mockContext);
            expect(result.success).toBe(false);
            expect(isPermissionDeniedError(result.error)).toBe(true);
        });
    });
    ...
    describe('executeRbacCreatePolicy', () => {
        ...
        it('fails create without role', async () => {
            const result = await executeRbacCreatePolicy({ action: 'create', effect: 'deny', resources: ['registry'], actions: ['approve'] }, mockContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain('role is required');
        });
        it('fails create without resources/actions', async () => {
            const result = await executeRbacCreatePolicy({ action: 'create', role: 'admin', effect: 'deny' }, mockContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain('resources and actions are required');
        });
        it('fails create without effect', async () => {
            const result = await executeRbacCreatePolicy({ action: 'create', role: 'admin', resources: ['registry'], actions: ['approve'] }, mockContext);
            expect(result.success).toBe(false);
        ...
        it('delete clears grants written by create', async () => {
            ...
            const result = await executeRbacCreatePolicy({ action: 'delete', role: 'member', resources: ['registry'], actions: ['approve'] }, mockContext);
            expect(result.success).toBe(true);
