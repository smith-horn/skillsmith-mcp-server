    ...
    return {
        async gatherData(periodDays, includeUserActivity) {
            ...
            const totalEventsRow = db
                .prepare('SELECT COUNT(*) as c FROM audit_logs WHERE timestamp >= ?')
                .get(since);
            const totalEvents = totalEventsRow?.c ?? 0;
            const byEventType = db
                .prepare('SELECT event_type, COUNT(*) as count FROM audit_logs ' +
            ...
            if (includeUserActivity) {
                const actors = db
                    .prepare('SELECT actor, COUNT(*) as actions, MAX(timestamp) as last_active ' +
                    'FROM audit_logs WHERE timestamp >= ? AND actor IS NOT NULL ' +
                    'GROUP BY actor ORDER BY actions DESC')
                    .all(since);
                const topTools = db
                    .prepare('SELECT resource, COUNT(*) as count FROM audit_logs ' +
                ...
                const activeDaysRow = db
                    .prepare('SELECT COUNT(DISTINCT DATE(timestamp)) as days FROM audit_logs ' +
                    'WHERE timestamp >= ?')
                    .get(since);
                userActivity = {
...
//# sourceMappingURL=compliance-tools.service.js.map