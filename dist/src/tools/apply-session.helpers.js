...
export function selectUndoTargets(opts) {
    if (opts.suggestionId !== undefined) {
        for (let i = sessionApplies.length - 1; i >= 0; i--) {
            if (sessionApplies[i].suggestionId === opts.suggestionId)
                return [sessionApplies[i]];
        }
        return [];
    }
 ...
 */
export async function resolveBackupFileName(backupRef, targetPath) {
    let entries;
    try {
    ...
    if (entries.includes(targetBase))
        return targetBase;
    if (entries.includes('SKILL.md'))
        return 'SKILL.md';
    return null;
}
//# sourceMappingURL=apply-session.helpers.js.map