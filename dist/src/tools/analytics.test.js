...
vi.mock('./team-resolver.js', () => ({
    resolveUserAccessToken: (...args) => mockResolveUserAccessToken(...args),
    resolveLicenseTeamId: (...args) => mockResolveLicenseTeamId(...args),
}));
    ...
    return {
        ...
        SupabaseAnalyticsService: function MockSupabaseAnalyticsService() {
            return {
                ...
                getReportingCoverage: (...args) => mockGetReportingCoverage(...args),
            };
        },
    };
});
...
// Imports after mocks
// ============================================================================
import { executeTeamAnalyticsDashboard, executeTeamUsageReport, executeAnalyticsDashboard, executeUsageReport, teamAnalyticsDashboardInputSchema, teamUsageReportInputSchema, analyticsDashboardInputSchema, usageReportInputSchema, } from './analytics.js';
...
describe('analytics tools', () => {
    ...
    describe('AC-9: all four tools include the data coverage note', () => {
        ...
        ])('%s includes the Data Coverage note and skill-invocation honesty line', async (_name, run) => {
            const result = await run();
            expect(result).toContain('## Data Coverage');
            expect(result).toContain(buildCoverageNote(FULL_COVERAGE));
            expect(result).toContain('Skill-invocation data: not yet captured for teams (Claude Code hook invocations are not team-attributed).');
        });
    });
    describe('data coverage note (buildCoverageNote rendered verbatim)', () => {
        it.each([
            ['full', { ...FULL_COVERAGE, coverageLevel: 'full' }],
            ['aggregate', { ...FULL_COVERAGE, coverageLevel: 'aggregate' }],
            ['qualitative', { ...FULL_COVERAGE, coverageLevel: 'qualitative' }],
    ...
    describe('skill-invocation honesty line', () => {
        it('is present and carries no counts or table — just the static sentence', async () => {
            const input = teamAnalyticsDashboardInputSchema.parse({});
            const result = await executeTeamAnalyticsDashboard(input, mockContext);
            const line = 'Skill-invocation data: not yet captured for teams (Claude Code hook invocations are not team-attributed).';
    ...
    describe('emit-health disclosure ("This Session" — session-scoped, D-8)', () => {
        ...
        it('falls back to a generic undelivered-count message for an unmapped reason', async () => {
            ...
            const input = teamAnalyticsDashboardInputSchema.parse({});
            const result = await executeTeamAnalyticsDashboard(input, mockContext);
            expect(result).toContain('3 events could not be delivered this session (reason: invalid_jwt).');
        });
    ...
    describe('team scoping: resolveLicenseTeamId is the only source of teamId', () => {
        ...
        it.each(ALL_TOOLS)('%s forwards the resolved teamId (not a hardcoded one) to both cloud calls', async (_name, run) => {
            ...
            await run();
            expect(mockGetToolUsage).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it.each(ALL_TOOLS)('%s re-resolves credentials per invocation — a later call never inherits an earlier one', async (_name, run) => {
        ...
        it.each(ALL_TOOLS)('%s renders honest zeros and an unavailable-coverage line when the caller is not a member of the resolved team', async (_name, run) => {
