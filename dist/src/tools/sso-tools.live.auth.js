 ...
 */
async function bindUserAuth(gate, noUserMessage) {
 ...
 */
export async function getSsoManageUserClient(operation) {
    return bindUserAuth('manage_sso', `Signing in is required to ${operation}. Configuring SSO checks the "team:manage_sso" ` +
 ...
 */
export async function getSsoReadUserClient(operation) {
    return bindUserAuth('read', `Signing in is required to ${operation}. This runs as you, not as your team's shared license ` +
        'key — a license key identifies a team, not a person. Run `skillsmith login` on this ' +
        'machine and retry.');
}
//# sourceMappingURL=sso-tools.live.auth.js.map