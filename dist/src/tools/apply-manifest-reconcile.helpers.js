 ...
 */
export function resolveReconcileScope(input) {
 ...
 */
export function reconcileKeyForLedgerEntry(name, client) {
    return manifestKeyFor(name, client);
}
// ============================================================================
 ...
 */
export async function assertDropTargetNoLongerResolves(name, entry) {
 ...
 */
export async function assertBackupTargetIsFile(filePath) {
    let stat;
    try {
 ...
 */
export async function takeManifestBackup(manifestPath) {
 ...
 */
export async function verifyEntryAgainstRegistry(entry, context, onQuotaExceeded, quotaAlreadyExhausted) {
    if (!entry.installPath) {
 ...
 */
export async function validateRelinkIdentity(id, context) {
    if (context.apiClient.isOffline()) {
...
//# sourceMappingURL=apply-manifest-reconcile.helpers.js.map