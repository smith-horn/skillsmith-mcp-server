...
export const applyNamespaceRenameToolSchema = {
    ...
    description: "[Skillsmith — Maintain stage] Apply a rename suggestion from a prior `skill_inventory_audit`. MUTATES `~/.claude` (renames a skill/command/agent file) — but ONLY when `confirmed: true`. Without `confirmed`, returns a non-mutating preview ({ preview: true, before, after, applied: false }) so the agent can show the change before committing. `action: 'apply'` uses the suggested name; `'custom'` uses `customName`; `'skip'` records a no-op.",
    inputSchema: {
        type: 'object',
        properties: {
            auditId: {
            ...
            action: {
                type: 'string',
                description: "'apply' uses the suggested name; 'custom' uses customName; 'skip' is a no-op.",
            ...
            confirmed: {
                type: 'boolean',
                description: 'When true, performs the file rename. When omitted/false, returns a non-mutating preview. Defaults to false.',
            },
        },
...
async function applyNamespaceRenameImpl(input) {
    ...
    const renameRequest = {
        suggestion,
        request: validInput.action === 'custom'
