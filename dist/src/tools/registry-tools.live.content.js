 ...
 */
export async function isTeamEnterpriseEntitled(teamId) {
    ...
    if (!sub)
        return { entitled: false, detail: 'subscription_not_found' };
    if (sub.tier !== REQUIRED_TIER)
        return { entitled: false, detail: 'team_tier_not_enterprise' };
 ...
 */
export async function getSkillContent(params) {
    ...
    if (metadata.error) {
        ...
        throw new Error(`Failed to read registry skill content: ${metadata.error.message ?? 'unknown error'}`);
    }
    ...
    if (!entitlement.entitled) {
        ...
        throw new Error(`Installing "${skillId}" from the private registry requires an active Enterprise ` +
    ...
    if (contentResp.error) {
        ...
        throw new Error(`Failed to read registry skill content: ${contentResp.error.message ?? 'unknown error'}`);
    }
...
//# sourceMappingURL=registry-tools.live.content.js.map