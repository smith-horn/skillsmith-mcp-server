 ...
 */
export function createRealAnalyticsService(db) {
    return {
        getDashboardData(periodDays) {
            ...
            const uniqueRow = db
                .prepare('SELECT COUNT(DISTINCT resource) as count FROM audit_logs WHERE timestamp >= ?')
                .get(since);
            const uniqueTools = uniqueRow?.count ?? 0;
            const topTools = db
                .prepare('SELECT resource as tool, COUNT(*) as count FROM audit_logs ' +
                'WHERE timestamp >= ? AND resource IS NOT NULL ' +
                'GROUP BY resource ORDER BY count DESC LIMIT 10')
                .all(since);
            const dailyTrend = db
                .prepare('SELECT DATE(timestamp) as date, COUNT(*) as count FROM audit_logs ' +
            ...
            const previousRow = db
                .prepare('SELECT COUNT(*) as count FROM audit_logs WHERE timestamp >= ? AND timestamp < ?')
                .get(previousSince, since);
            const previous = previousRow?.count ?? 0;
            const changePercent = previous > 0 ? Math.round(((totalToolCalls - previous) / previous) * 100) : 0;
            return {
                totalToolCalls,
                uniqueTools,
                topTools,
        ...
        getUsageReport(periodDays, detailed) {
            ...
            const since = new Date(Date.now() - periodDays * 86_400_000).toISOString();
            const byActor = db
                .prepare('SELECT actor, COUNT(*) as count FROM audit_logs ' +
