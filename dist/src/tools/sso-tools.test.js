...
describe('sso-tools', () => {
    ...
    describe('configure_sso action "claim_domain"', () => {
        it('should fail without a domain', async () => {
            const result = await executeConfigureSso({ action: 'claim_domain', protocol: 'saml' }, mockContext);
            expect(result.success).toBe(false);
    ...
    describe('configure_sso action "verify_domain"', () => {
        it('should fail without a domain', async () => {
            const result = await executeConfigureSso({ action: 'verify_domain', protocol: 'saml' }, mockContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain('domain is required');
        });
