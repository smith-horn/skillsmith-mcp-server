 ...
 */
export async function quiesceBackgroundSync(bg) {
    if (!bg)
        return;
    let settled = false;
    await Promise.race([
        ...
        new Promise((resolve) => setTimeout(resolve, SHUTDOWN_QUIESCE_TIMEOUT_MS)),
    ]);
    if (!settled) {
        logger.error('[skillsmith] Background sync did not quiesce within the shutdown bound; proceeding to close db', {});
    }
}
//# sourceMappingURL=index.shutdown-helpers.js.map