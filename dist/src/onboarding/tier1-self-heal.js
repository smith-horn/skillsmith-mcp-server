...
import { SKILLSMITH_DIR, TIER1_SKILLS, formatWelcomeMessage, } from './first-run.js';
/**
 ...
 */
export function isTier1AutoInstallDisabled() {
 ...
 */
export function readTier1Status() {
    try {
        const parsed = JSON.parse(readFileSync(TIER1_STATUS_FILE, 'utf-8'));
        return {
            installed: Array.isArray(parsed.installed)
                ? parsed.installed.filter((s) => typeof s === 'string')
                : [],
 ...
 */
export function writeTier1Status(status) {
    try {
        ...
        console.error('[skillsmith] Failed to persist Tier-1 status:', error instanceof Error ? error.message : 'Unknown error');
    }
}
/**
 ...
 */
function countsAsInstalled(result) {
    if (result.success)
        return true;
    ...
    if (errorCode === 'ALREADY_INSTALLED')
        return true;
...
export async function maybeInstallMissingTier1Skills(toolContext, options = {}) {
    ...
    if (isThrottled(status.lastAttempt, Date.now()))
        return;
    console.error(`[skillsmith] Installing ${missing.length} missing Tier-1 skill(s): ${missing
        .map((s) => s.name)
        .join(', ')}`);
    ...
    for (const skill of missing) {
        try {
            const result = await installSkill({ skillId: skill.id, force: false, skipScan: false, skipOptimize: false, confirmed: true }, toolContext);
            if (countsAsInstalled(result)) {
                ...
                console.error(`[skillsmith] Failed to install ${skill.name}: ${result.error ?? 'Unknown error'}`);
            }
        }
        catch (error) {
            failures.push(skill.name);
    ...
    const registrySkills = TIER1_SKILLS.filter((skill) => mergedInstalled.includes(skill.name)).map((skill) => ({ name: skill.name, attribution: skill.id.split('/')[0] }));
    const skillInfos = [
...
//# sourceMappingURL=tier1-self-heal.js.map