...
describe('installGlobalCrashHandlers (SMI-5787)', () => {
    ...
    it('logs to disk (via logger.error) and exits 1 on an uncaught exception', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith('[skillsmith] Uncaught exception — server exiting', { err: error });
    ...
    it('logs to disk and exits 1 on an unhandled rejection with an Error reason', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith('[skillsmith] Unhandled promise rejection — server exiting', { err: error });
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
    it('wraps a non-Error rejection reason in an Error before logging', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith('[skillsmith] Unhandled promise rejection — server exiting', { err: expect.objectContaining({ message: 'string reason' }) });
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
});
//# sourceMappingURL=shutdown-crash-handlers.test.js.map