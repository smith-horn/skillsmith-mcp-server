...
import { createDatabaseAsync, openDatabaseAsync, initializeSchema, SearchService, SkillRepository, validateDbPath, SkillsmithApiClient, initializePostHog, shutdownPostHog, getOrCreateInstallId, SyncConfigRepository, SyncHistoryRepository, SyncEngine, SkillVersionRepository, CoInstallRepository, SkillDependencyRepository, BackgroundSyncService, getApiKey, loadCredentials, tryRefreshToken, resolveFreshAccessToken, setTelemetryIdentityProvider, setTelemetryIdentityInvalidationHandler, } from '@skillsmith/core';
 ...
 */
let cachedTelemetryIdentity = null;
let telemetryIdentityRefreshTimer;
/**
...
export function _getCachedTelemetryIdentityForTests() {
    return cachedTelemetryIdentity;
}
async function refreshTelemetryIdentity(currentApiKey) {
    ...
    if (generation !== telemetryIdentityGeneration)
        return;
...
export async function createToolContextAsync(options = {}) {
    ...
    telemetryIdentityRefreshTimer = setInterval(() => void refreshTelemetryIdentity(apiKey), 5 * 60 * 1000);
