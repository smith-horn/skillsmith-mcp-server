...
describe('runSecurityAudit', () => {
    ...
    describe('SMI-5207: sensitive_path SCANNER_RULESET_VERSION bump + comparable gate', () => {
        const ICM_SHIPWRIGHT_DESCRIPTION = 'Make AI-agent workspaces safe to run and ship. Folders + markdown replace framework ' +
        ...
        it('an old-ruleset-version `malicious`/HIGH baseline for a now-cleared sensitive_path FP is treated as STALE, not authoritative — re-scan reduces serious findings to zero', async () => {
                            ...
                            findings: [
                                {
                                    type: 'sensitive_path',
                                    severity: 'high',
                                    message: 'Reference to potentially sensitive path: "secret/PII" (\\bsecrets?\\/[a-z0-9_.-]+)',
                                    inDocumentationContext: false,
                                },
                            ],
                            riskBreakdown: { ...ZERO_BREAKDOWN },
            ...
            const seriousNow = refreshed?.report.findings.filter((f) => f.severity === 'high' || f.severity === 'critical').length;
        ...
        it('reduces (not eliminates) the "serious findings" count when the skill is genuinely still malicious for an unrelated reason — the sensitive_path downgrade never touches a real co-located threat', async () => {
            ...
            expect(refreshed?.report.findings.some((f) => f.type === 'sensitive_path' && f.severity === 'high')).toBe(false);
            expect(refreshed?.report.findings.some((f) => f.type === 'prompt_leaking' && f.severity === 'critical')).toBe(true);
