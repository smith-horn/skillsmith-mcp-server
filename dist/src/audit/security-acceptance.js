...
import { ACCEPTANCE_STORE_VERSION, MAX_RECORDS, MAX_STORE_BYTES, emptyAcceptanceStore, } from './security-acceptance.types.js';
/** Absolute path to the acceptance store, `~/.skillsmith/audits/security-acceptance.json`. Mirrors `defaultBaselinePath()`. */
export function defaultAcceptancePath(homeDir = os.homedir()) {
 ...
 */
export function fingerprintTuple(f) {
    return [
        f.type,
        f.severity,
        f.category ?? null,
        f.message,
        f.location ?? null,
        Number.isInteger(f.lineNumber) ? f.lineNumber : null,
        f.filePath ?? null,
        f.evidenceType ?? null,
...
export function isValidAcceptKeyFormat(key) {
 ...
 */
function readStoreBounded(storePath) {
    let fd;
    try {
...
function isValidAcceptanceRecord(r) {
    if (typeof r !== 'object' || r === null)
        return false;
    const o = r;
    if (!isValidAcceptKeyFormat(String(o['acceptKey'])))
        return false;
    if (typeof o['sourcePath'] !== 'string' || typeof o['identifier'] !== 'string')
        return false;
    if (!ACCEPT_KEY_RE.test(String(o['contentDigest'])) ||
        !ACCEPT_KEY_RE.test(String(o['findingFingerprint'])))
        return false;
    if (typeof o['rulesetVersion'] !== 'string')
        return false;
    ...
    if (computeAcceptKey({
        ...
        findingFingerprint: o['findingFingerprint'],
        rulesetVersion: o['rulesetVersion'],
    }) !== o['acceptKey'])
        return false;
    if (typeof o['acceptedAt'] !== 'string' || Number.isNaN(Date.parse(o['acceptedAt'])))
        return false;
    ...
    if (d['location'] !== null && typeof d['location'] !== 'string')
        return false;
    if (d['lineNumber'] !== null && typeof d['lineNumber'] !== 'number')
        return false;
    return true;
}
/**
 ...
 */
export function loadAcceptanceStore(storePath) {
    const warnings = [];
    ...
    if (bounded.kind === 'errored') {
        warnings.push({
            ...
            message: 'The acceptance store could not be read (a filesystem error occurred) -- treating it as empty. Findings that were accepted will re-appear; repair or remove the store to recover them.',
        });
        return { store: emptyAcceptanceStore(), warnings };
    }
    ...
    try {
        ...
        warnings.push({
            code: 'acceptance_store_unreadable',
            message: 'The acceptance store could not be parsed as JSON -- treating it as empty. Findings that were accepted will re-appear; repair or remove the store to recover them.',
        });
        return { store: emptyAcceptanceStore(), warnings };
    }
    if (typeof parsed !== 'object' ||
        parsed === null ||
        parsed['version'] !== ACCEPTANCE_STORE_VERSION ||
        typeof parsed['revision'] !== 'number' ||
        typeof parsed['storeDigest'] !== 'string' ||
        !Array.isArray(parsed['records'])) {
        warnings.push({
            code: 'acceptance_store_unreadable',
            message: 'The acceptance store has an unrecognized shape or version -- treating it as empty. Findings that were accepted will re-appear; repair or remove the store to recover them.',
        });
        return { store: emptyAcceptanceStore(), warnings };
    }
    const shell = parsed;
    ...
    if (expectedDigest !== shell.storeDigest) {
        warnings.push({
            ...
            message: 'The acceptance store failed its self-consistency check (a truncated write or hand-edit likely left it stale) -- treating it as empty. Findings that were accepted will re-appear; repair or remove the store to recover them.',
        });
        return { store: emptyAcceptanceStore(), warnings };
    }
    const validRecords = [];
    ...
    validRecords.sort((a, b) => a.acceptedAt < b.acceptedAt ? 1 : a.acceptedAt > b.acceptedAt ? -1 : 0);
...
//# sourceMappingURL=security-acceptance.js.map