 ...
 */
export async function writeAuditHistory(result, opts = {}) {
 ...
 */
export async function readAuditHistory(auditId, opts = {}) {
    const auditsDir = opts.auditsDir ?? DEFAULT_AUDITS_DIR;
 ...
 */
export function deriveCollisionId(auditId, entries) {
    ...
    const claudeMdSuffix = sortedClaudeMdIdentifiers.length > 0 ? `:${sortedClaudeMdIdentifiers.join(',')}` : '';
 ...
 */
export function hasClaudeMdEntries(flag) {
    return flag.entries.some((e) => e.kind === 'claude_md_rule');
}
//# sourceMappingURL=audit-history.js.map