...
import { sendAuditDigest, getAuditNotifyState, recordAuditNotify, shouldAutoPush, loadCredentials, } from '@skillsmith/core';
 ...
 */
export function buildAuditDigestPayload(result) {
 ...
 * identical digest.
 */
export function hashDigest(payload) {
 ...
 * @see SMI-5541
 */
export async function maybeAutoNotifyAudit(opts) {
    if (process.env.SKILLSMITH_AUDIT_EMAIL_DISABLE === '1')
        return null;
    ...
    try {
        ...
        if (!shouldAutoPush(now, state.lastNotifyAt))
            return null;
        ...
        console.error('[skillsmith] audit auto-notify failed:', error instanceof Error ? error.message : error);
        return null;
    }
}
//# sourceMappingURL=audit-notify.js.map