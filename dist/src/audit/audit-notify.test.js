...
import { buildAuditDigestPayload, hashDigest, maybeAutoNotifyAudit, MAX_DIGEST_FINDINGS, } from './audit-notify.js';
function finding(verdict, identifier) {
    return {
        kind: 'security',
...
describe('buildAuditDigestPayload', () => {
    it('maps findings to the compact shape and takes counts from the summary', () => {
        const payload = buildAuditDigestPayload(result([finding('hostile', 'a')], { scanned: 9, hostile: 4, malicious: 2, suspicious: 3 }));
    ...
    it('sorts findings strongest-first (hostile < malicious < suspicious)', () => {
        const payload = buildAuditDigestPayload(result([finding('suspicious', 's'), finding('hostile', 'h'), finding('malicious', 'm')]));
...
describe('hashDigest', () => {
    ...
    it('changes when a summary COUNT changes even if the (capped) findings list does not', () => {
        const findings = [{ identifier: 'a', kind: 'skill', verdict: 'hostile', reason: 'r' }];
...
describe('maybeAutoNotifyAudit', () => {
    ...
    afterEach(() => {
        if (savedDisable !== undefined)
            process.env.SKILLSMITH_AUDIT_EMAIL_DISABLE = savedDisable;
        else
            delete process.env.SKILLSMITH_AUDIT_EMAIL_DISABLE;
        vi.restoreAllMocks();
    });
    ...
    it('dedups an identical picture: skips the send, backs off WITHOUT re-recording the hash', async () => {
        ...
        const hash = mocks.recordAuditNotify.mock.calls[0]?.[1];
...
//# sourceMappingURL=audit-notify.test.js.map