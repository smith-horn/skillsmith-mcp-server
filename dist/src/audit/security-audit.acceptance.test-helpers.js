...
export function report(skillId, opts) {
    return {
        skillId,
        passed: opts.passed,
...
export function entry(identifier, sourcePath) {
    return {
        kind: 'skill',
        identifier,
...
export function seedAcceptAll(acceptancePath, contentDigest, findings, reason = 'reviewed') {
    const records = findings.map((f) => ({
        ...
        display: {
            ...
            lineNumber: Number.isInteger(f.lineNumber) ? f.lineNumber : null,
        },
        acceptedAt: '2026-07-01T00:00:00.000Z',
        reason,
    }));
    const shell = { version: ACCEPTANCE_STORE_VERSION, revision: 1, records };
    const store = { ...shell, storeDigest: computeStoreDigest(shell) };
...
//# sourceMappingURL=security-audit.acceptance.test-helpers.js.map