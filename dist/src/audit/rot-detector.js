 ...
 */
export async function detectRot(inventory, opts = {}) {
    ...
    for (const entry of inventory) {
        ...
        if (content === null)
            continue; // unreadable — fail toward no finding, never a crash.
 ...
 */
function compareBySourcePathThenSignal(a, b) {
 ...
 * comparison.
 */
async function detectVersionDrift(inventory, opts) {
    void inventory;
    void opts;
    return [];
}
 ...
 */
function findDeadRefReason(content) {
...
function buildFinding(auditId, entry, signal, severity, reason) {
    return {
        kind: 'rot',
 ...
 */
function deriveRotId(auditId, entry, signal) {
    const input = `${auditId}:${entry.source_path}:${signal}`;
    return crypto.createHash('sha256').update(input).digest('hex').slice(0, 16);
}
//# sourceMappingURL=rot-detector.js.map