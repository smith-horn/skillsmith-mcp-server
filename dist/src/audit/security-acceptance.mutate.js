 ...
 */
function testDelayIfConfigured() {
...
function gcExpiredRecords(records) {
 ...
 */
function mutateStoreUnderLock(acceptancePath, mutate, onBeforePreReadCommitted) {
    ...
    try {
        ...
        if (loaded.revision !== preRead.revision) {
            warnings.push({
                ...
                message: 'The acceptance store changed between an earlier read and this write -- another process (or a lock-bypassing hand-edit) touched it concurrently. This write still proceeds against the CURRENT contents (no data is lost), but investigate if this recurs.',
            });
        }
        ...
        const full = { ...next, storeDigest: computeStoreDigest(next) };
        try {
 ...
 */
export function acceptFinding(acceptancePath, record, _onBeforeLock) {
...
export function revokeAcceptance(acceptancePath, acceptKey) {
    const result = mutateStoreUnderLock(acceptancePath, (records) => {
        if (!records.some((r) => r.acceptKey === acceptKey))
            return null;
        return records.filter((r) => r.acceptKey !== acceptKey);
    });
    if (result.notFound)
        return { ok: false, code: 'key_not_found', warnings: result.warnings };
    return { ok: result.ok, warnings: result.warnings };
}
//# sourceMappingURL=security-acceptance.mutate.js.map