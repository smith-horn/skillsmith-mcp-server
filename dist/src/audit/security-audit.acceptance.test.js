...
import { CRITICAL_EXFIL, entry, report, seedAcceptAll, sha256, } from './security-audit.acceptance.test-helpers.js';
let tmpDir;
let baselinePath;
let acceptancePath;
beforeEach(() => {
...
describe('security acceptance -- H-5/H-6a: suppression + INV-5 visibility', () => {
    it('H-5: accepting all findings on a failing skill suppresses it into an ACCEPTED annotation, never silently', async () => {
        ...
        seedAcceptAll(acceptancePath, sha256(content), [CRITICAL_EXFIL], 'reviewed 2026-07-29: documented example, not a real payload');
        const res = await runSecurityAudit({
            baselinePath,
            acceptancePath,
            inventory: [e],
    ...
    it('a PARTIALLY accepted skill (one of two findings) is NOT suppressed', async () => {
        ...
        const other = { type: 'ssrf', severity: 'high', message: 'other finding' };
        ...
        const res = await runSecurityAudit({
            ...
            inventory: [e],
            readContent: () => content,
            scan: () => report('evil', { passed: false, riskScore: 80, findings: [CRITICAL_EXFIL, other] }),
        });
        expect(res.findings[0]?.accepted).toBeUndefined();
...
//# sourceMappingURL=security-audit.acceptance.test.js.map