...
# Procedure`;
        ...
        expect(result.description).toBe('Generates and formats changelog files for a new release based on provided version and raw changelog data.');
    });
...
# PR Creator`;
        ...
        expect(result.description).toBe("Use this skill when asked to create a pull request (PR). It ensures all PRs follow the repository's established templates and standards.");
    });
...
describe('localSkillToSearchResult: SMI-2760 compatibility propagation', () => {
    ...
    it('should omit compatibility when empty array', () => {
        const skill = { ...baseLocalSkill, compatibility: [] };
        const result = localSkillToSearchResult(skill);
        expect(result.compatibility).toBeUndefined();
    });
});
