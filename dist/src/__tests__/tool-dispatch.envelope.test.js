...
describe('dispatchToolCall validation envelope (SMI-4313)', () => {
    ...
    describe('direct dispatch sites — bogus payload returns structured envelope', () => {
        it.each(DIRECT_DISPATCH_CASES)('$tool: wrong-type payload returns isError + ValidationError envelope', async ({ tool, payload }) => {
            const result = await dispatchToolCall(tool, payload, {}, licenseMiddleware, quotaMiddleware);
        ...
        it('uninstall_skill: undefined args → envelope, not throw', async () => {
            ...
            const result = await dispatchToolCall('uninstall_skill', undefined, {}, licenseMiddleware, quotaMiddleware);
            expect(result.isError).toBe(true);
            const body = parseEnvelope(result);
            expect(body.tool).toBe('uninstall_skill');
        });
        it('skill_suggest: validation fails before license/quota check runs', async () => {
            await dispatchToolCall('skill_suggest', { query: 123 }, {}, licenseMiddleware, quotaMiddleware);
    ...
    describe('gated sites via withLicenseAndQuota — envelope on invalid input', () => {
        it.each(GATED_CASES)('$tool: wrong-type payload short-circuits to ValidationError envelope', async ({ tool, payload }) => {
            const result = await dispatchToolCall(tool, payload, {}, licenseMiddleware, quotaMiddleware);
        ...
        it('gated validation short-circuits before license check', async () => {
            ...
            await dispatchToolCall('skill_diff', { skillId: 123 }, {}, licenseMiddleware, quotaMiddleware);
    ...
    describe('envelope shape snapshot (skill_compare)', () => {
        it('matches the documented envelope shape', async () => {
            const result = await dispatchToolCall('skill_compare', { skillIds: 'not-an-array' }, {}, licenseMiddleware, quotaMiddleware);
            expect(result.isError).toBe(true);
            const body = parseEnvelope(result);
...
//# sourceMappingURL=tool-dispatch.envelope.test.js.map