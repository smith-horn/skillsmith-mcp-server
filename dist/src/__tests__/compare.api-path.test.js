...
describe('executeCompare (API path, SMI-5896)', () => {
    it('resolves both skills via the API when neither exists in the local DB', async () => {
        ...
        vi.spyOn(context.apiClient, 'getSkill').mockImplementation(async (id) => {
        ...
        const result = await executeCompare({ skill_a: 'anthropic/commit', skill_b: 'community/jest-helper' }, context);
    ...
    it('resolves skill_a via the API and falls back to the local DB for skill_b', async () => {
        ...
        const result = await executeCompare({ skill_a: 'anthropic/commit', skill_b: 'local/only-skill' }, context);
        expect(result.comparison.a.name).toBe('commit');
    ...
    it('reports SKILL_NOT_FOUND for skill_a when both API and local DB miss it', async () => {
        ...
        await expect(executeCompare({ skill_a: 'nobody/nothing', skill_b: 'also/nobody' }, context)).rejects.toThrow(SkillsmithError);
        ...
        await expect(executeCompare({ skill_a: 'nobody/nothing', skill_b: 'also/nobody' }, context)).rejects.toMatchObject({
            details: { id: 'nobody/nothing' },
        });
    });
});
//# sourceMappingURL=compare.api-path.test.js.map