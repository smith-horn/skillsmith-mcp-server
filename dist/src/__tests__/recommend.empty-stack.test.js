...
beforeEach(async () => {
    ...
    vi.spyOn(context.apiClient, 'getRecommendations').mockRejectedValue(new Error('[test] getRecommendations should not be called for an empty derived stack'));
});
afterEach(async () => {
...
describe('executeRecommend — empty derived stack guard (SMI-5896)', () => {
    ...
    it('surfaces the shared empty-stack guidance under `suggestion`', async () => {
        const result = await executeRecommend({ installed_skills: [], limit: 5 }, context);
        expect(result.suggestion).toBe('No technology stack could be derived for recommendations — this usually ' +
    ...
    it('does not derive an empty stack when project_context supplies usable words', async () => {
        ...
        const result = await executeRecommend({ installed_skills: [], project_context: 'react testing library', limit: 5 }, context);
    ...
    it('tracks the recommend event with result_count: 0 when distinctId is set', async () => {
        ...
        const contextWithId = { ...context, distinctId: 'empty-stack-test-user' };
        await executeRecommend({ installed_skills: [], limit: 5 }, contextWithId);
        expect(trackSpy).toHaveBeenCalledWith('empty-stack-test-user', 'skill_recommend', expect.objectContaining({ result_count: 0, source: 'mcp' }));
    });
});
//# sourceMappingURL=recommend.empty-stack.test.js.map