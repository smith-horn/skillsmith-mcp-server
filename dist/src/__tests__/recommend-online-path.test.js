...
describe('Recommend Tool - Online API Path (SMI-2755)', () => {
    ...
    it('(SMI-5986) includes a short technical term from project_context in the outbound stack', async () => {
        ...
        expect(getRecommendationsSpy).toHaveBeenCalledWith(expect.objectContaining({ stack: expect.arrayContaining(['git']) }));
    });
    ...
    it('(SMI-5986) handles punctuation and case variants in project_context ("Git", "git,")', async () => {
        ...
        const call = getRecommendationsSpy.mock.calls[0]?.[0];
    ...
    it('(SMI-5178) default installable_only filters out discovery-only recommendations', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (r) => r.skill_id === 'community/discovery-only');
    ...
    it('(SMI-5178) installable_only: false opts out — discovery-only included', async () => {
        ...
        const result = await executeRecommend({ project_context: 'git', limit: 5, installable_only: false }, onlineContext);
