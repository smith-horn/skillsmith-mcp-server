...
import { resolveSearchLimit, DEFAULT_SEARCH_LIMIT, MIN_SEARCH_LIMIT, MAX_SEARCH_LIMIT, } from '../tools/search.helpers.js';
...
describe('search `limit` — unvalidated-boundary regression (SMI-5896)', () => {
    ...
    it('returns the default page (not zero results) when a client sends limit: null', async () => {
        ...
        const result = await executeSearch({ query: 'widget', limit: null }, context);
    ...
    it('returns the default page when a client sends a non-numeric limit', async () => {
        const result = await executeSearch({ query: 'widget', limit: 'lots' }, context);
        expect(result.results.length).toBe(DEFAULT_SEARCH_LIMIT);
    });
});
...
//# sourceMappingURL=search-limit.test.js.map