...
function createMockLicenseMiddleware() {
    return {
...
function createMockQuotaMiddleware() {
    return {
...
describe('dispatchToolCall', () => {
    ...
    describe('SMI-3913: comingSoon handler for unmapped tools', () => {
        ...
        it.each(unmappedGatedTools)('should return comingSoon for unmapped gated tool "$name"', async ({ name, feature }) => {
            const result = await dispatchToolCall(name, {}, {}, licenseMiddleware, quotaMiddleware);
        ...
        it('should throw Unknown tool for tools not in TOOL_FEATURES', async () => {
            await expect(dispatchToolCall('totally_unknown_tool', {}, {}, licenseMiddleware, quotaMiddleware)).rejects.toThrow('Unknown tool: totally_unknown_tool');
        });
...
//# sourceMappingURL=tool-dispatch.test.js.map