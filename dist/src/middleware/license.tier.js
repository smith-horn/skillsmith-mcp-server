 ...
 */
import { getApiBaseUrl, resolveSessionTier, SessionTierAuthError, SessionTierTransientError, } from '@skillsmith/core';
import { FEATURE_TIERS } from './toolFeatureMapping.js';
/**
...
export function featuresForTier(tier) {
    ...
    return Object.keys(FEATURE_TIERS).filter((flag) => TIER_RANK[FEATURE_TIERS[flag]] <= ceiling);
}
/**
...
function buildCommunityLicense() {
 ...
 */
function handleTransientFailure(context, reason, logPrefix) {
    const shortExpiry = Date.now() + STALE_ERROR_TTL_MS;
    const stale = context.cachedLicense;
    if (stale) {
        context.cacheExpiry = shortExpiry;
        console.warn(`[skillsmith] ${logPrefix} live tier check failed (${reason}); serving stale-cached ` +
    ...
    context.cacheExpiry = shortExpiry;
    console.warn(`[skillsmith] ${logPrefix} live tier check failed (${reason}) with no prior cache; ` +
 ...
 */
export function createTierResolver(context, cacheTtlMs) {
    return async function resolveTierViaApiKey(apiKey) {
        ...
        try {
            ...
            if (response.status === 429 || response.status >= 500) {
                return handleTransientFailure(context, response.status === 429 ? '429' : `5xx (${response.status})`, 'SMI-1953');
            }
            let body;
            try {
            ...
            if (response.ok && data?.authenticated === false) {
                ...
                context.cachedLicense = community;
                context.cacheExpiry = Date.now() + cacheTtlMs;
                console.warn('[skillsmith] SMI-1953 live tier resolved: community (API key not authenticated)');
                return community;
            }
            ...
            return handleTransientFailure(context, `unexpected-response (status ${response.status})`, 'SMI-1953');
        }
        catch {
            // fetch threw: network error, or AbortError from the timeout.
            return handleTransientFailure(context, controller.signal.aborted ? 'timeout' : 'network-error', 'SMI-1953');
        }
        finally {
 ...
 * @param cacheTtlMs - Full TTL for DEFINITIVE results.
 */
export function createSessionTokenResolver(context, cacheTtlMs) {
    return async function resolveTierViaSessionToken() {
        try {
            ...
            if (result.authenticated && result.tier) {
                ...
                context.cachedLicense = license;
                context.cacheExpiry = Date.now() + cacheTtlMs;
                console.warn(`[skillsmith] SMI-6098 live tier resolved: '${tier}' (via device-login session)`);
                return license;
            }
            ...
            context.cachedLicense = community;
            context.cacheExpiry = Date.now() + cacheTtlMs;
            console.warn('[skillsmith] SMI-6098 live tier resolved: community (session not authenticated)');
            return community;
        }
        catch (error) {
