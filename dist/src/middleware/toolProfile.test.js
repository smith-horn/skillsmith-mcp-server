...
import { AGENT_TOOL_PROFILE_ENV_VAR, AGENT_TOOL_PROFILE_NAMES, isAgentToolProfileActive, filterToolsForAgentProfile, } from './toolProfile.js';
    ...
    it('defaults to process.env when no env argument is passed', () => {
        ...
        try {
            ...
            if (savedEnv === undefined)
                ...
                process.env[AGENT_TOOL_PROFILE_ENV_VAR] = savedEnv;
        }
    });
});
...
describe('filterToolsForAgentProfile — SKILLSMITH_TOOL_PROFILE=agent', () => {
    ...
    it('is silently inert for a profile name with no matching registration', () => {
        ...
        expect(() => filterToolsForAgentProfile(TODAY_REGISTERED_TOOLS, { [AGENT_TOOL_PROFILE_ENV_VAR]: 'agent' })).not.toThrow();
    });
...
describe('tier-gating is unaffected by SKILLSMITH_TOOL_PROFILE (listing-only filter)', () => {
    ...
    it('gate still fires for a gated tool with the profile active', async () => {
        ...
        try {
            ...
            const result = await withLicenseAndQuota('skill_audit', { skillId: 'foo/bar' }, inputSchema, handler, mockCtx, denyingLicense(), mockQuota);
...
//# sourceMappingURL=toolProfile.test.js.map