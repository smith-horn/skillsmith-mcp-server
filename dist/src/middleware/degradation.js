 ...
 * are unavailable.
 */
import { createLicenseMiddleware, getRequiredFeature, } from './license.js';
 ...
 */
export const consoleDegradationLogger = {
    log: (event) => {
        const prefix = event.action === 'degraded' ? '[UPGRADE REQUIRED]' : `[${event.action.toUpperCase()}]`;
 ...
 */
function createUpgradePrompt(feature, _currentTier) {
 ...
 */
function createDetailedUpgradeMessage(feature, currentTier) {
    const displayName = FEATURE_DISPLAY_NAMES[feature];
...
export function getTierComparisonMessage() {
    return [
        ...
        'Individual ($9.99/month)',
        '----------------------------------------',
        '  - Version Tracking (check for skill updates)',
        '  - Basic Analytics',
        '  - Email Support',
        '',
        'Team ($25/user/month)',
 ...
 */
function createGracefulDegradationResponse(toolName, feature, currentTier, _validationResult) {
    const requiredTier = FEATURE_TIERS[feature];
    const displayName = FEATURE_DISPLAY_NAMES[feature];
    const response = {
 ...
 */
function getCommunityAlternatives(toolName) {
 ...
 * ```
 */
export function createDegradationMiddleware(options = {}) {
     ...
     */
    async function wrapHandler(toolName, handler, request) {
        ...
        return createGracefulDegradationResponse(toolName, requiredFeature, currentTier, validationResult);
    }
    /**
     * Create a wrapped handler function
     */
    function createWrappedHandler(toolName, handler) {
     ...
     */
    async function wouldDegrade(toolName) {
        const requiredFeature = getRequiredFeature(toolName);
        if (requiredFeature === null) {
            return false;
        }
     ...
     */
    async function getUpgradePrompt(toolName) {
        const requiredFeature = getRequiredFeature(toolName);
        if (requiredFeature === null) {
            return null;
        }
