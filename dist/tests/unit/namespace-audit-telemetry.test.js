...
import { emitAuditCompleteEvent, } from '../../src/tools/namespace-audit/telemetry.js';
...
describe('emitAuditCompleteEvent — short-circuit behaviour', () => {
    it('audit_mode=off → no fetch, returns null (decision #7 defense-in-depth)', async () => {
        const ret = await emitAuditCompleteEvent(resultWithEverything(), { ...baseCtx, audit_mode: 'off' }, { fetchImpl: fetchSpy });
        expect(ret).toBeNull();
...
describe('emitAuditCompleteEvent — preventative mode payload shape', () => {
    ...
    it('includes user_id when caller passes a hashed actor proxy', async () => {
        await emitAuditCompleteEvent(resultWithEverything(), { ...baseCtx, user_id: 'hashed-actor-deadbeef' }, { fetchImpl: fetchSpy });
        const body = JSON.parse(fetchSpy.mock.calls[0][1].body);
    ...
    it('swallows fetch errors — telemetry never breaks the audit', async () => {
        ...
        await expect(emitAuditCompleteEvent(resultWithEverything(), baseCtx, {
            fetchImpl: failing,
...
//# sourceMappingURL=namespace-audit-telemetry.test.js.map