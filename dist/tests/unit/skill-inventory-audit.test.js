 ...
 */
function plantSkill(testHome, identifier, description = 'unit-fixture') {
 ...
 */
function plantCommand(testHome, identifier) {
...
function isResponse(v) {
    return !('errorCode' in v && 'success' in v && v.success === false);
}
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
...
describe('skill_inventory_audit — exclusions filter', () => {
    it('filters matching collisions by default (applyExclusions defaults to true)', async () => {
        ...
        try {
            ...
            fs.writeFileSync(path.join(skillsmithDir, 'audit-exclusions.json'), JSON.stringify({
                version: 1,
...
describe('skill_inventory_audit — SMI-4737 token cap on target.identifier', () => {
    ...
    it('skips rename suggestion when target.identifier > 128 chars', async () => {
        ...
        try {
            ...
            const overCapSuggestion = response.renameSuggestions.find((s) => s.currentName === overCapName);
...
//# sourceMappingURL=skill-inventory-audit.test.js.map