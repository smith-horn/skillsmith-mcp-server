...
import { detectCollisions, detectExactCollisions, detectGenericTokenFlags, } from '../../src/audit/collision-detector.js';
...
describe('detectCollisions (orchestrator)', () => {
    ...
    it('exact-pass duration is recorded in passDurations.exact', async () => {
        const inv = Array.from({ length: 20 }, (_, i) => entry({ identifier: `s-${i}`, source_path: `/s/${i}` }));
        const result = await detectCollisions(inv);
...
//# sourceMappingURL=collision-detector.test.js.map