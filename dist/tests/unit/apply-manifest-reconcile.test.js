...
import { applyManifestReconcile, applyManifestReconcileInputSchema, } from '../../src/tools/apply-manifest-reconcile.js';
...
function plantSkill(name, content = `---\nname: ${name}\n---\nfixture body`) {
...
function writeManifest(entries) {
    ...
    const manifest = { version: '1.0.0', installedSkills: entries };
    fs.writeFileSync(MANIFEST_PATH, JSON.stringify(manifest, null, 2), 'utf-8');
}
function readManifest() {
    return JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf-8'));
}
function makeContext(opts = { online: true }) {
    return {
 ...
 * need to be.
 */
async function reconcile(input, context = makeContext()) {
...
describe('mark_local', () => {
    it('writes source:"unknown" and provenance:"local" atomically, backs up, and ledgers the change', async () => {
        ...
        const result = await reconcile({ action: 'mark_local', name: 'commit', reason: 'wrong id/source' }, makeContext());
...
describe('relink', () => {
    it('requires BOTH id and source (guard rejection)', async () => {
        writeManifest({ commit: makeEntry() });
        const result = await reconcile({ action: 'relink', name: 'commit', id: 'author/name' }, makeContext());
        expect(result.success).toBe(false);
    ...
    it('refuses with relink_unvalidated when the registry does not confirm the id', async () => {
        ...
        const result = await reconcile({ action: 'relink', name: 'commit', id: 'bogus/id', source: 'github:bogus/id' }, makeContext());
        expect(result.success).toBe(false);
...
describe('drop_entry', () => {
    it('removes an entry whose installPath no longer resolves, and ledgers a null afterState', async () => {
        ...
        const result = await reconcile({ action: 'drop_entry', name: 'shutdown-persistence-fixture' }, makeContext());
        expect(result.success).toBe(true);
...
describe('verify', () => {
    ...
    it('batches over every entry when name is omitted, writing only matched entries', async () => {
        ...
        mockedLookup.mockImplementation(async (id) => {
            if (id.includes('good')) {
                return {
    ...
    it('single-entry verify fails with verify_unavailable when offline', async () => {
        writeManifest({ commit: makeEntry() });
        const result = await reconcile({ action: 'verify', name: 'commit' }, makeContext({ online: false }));
        expect(result.success).toBe(false);
...
describe('revert', () => {
    it('round-trips mark_local', async () => {
        ...
        const reverted = await reconcile({ action: 'revert', ledgerEntryId: applied.ledgerEntryId }, makeContext());
        expect(reverted.success).toBe(true);
        expect(reverted.noOp).toBe(false);
        expect(readManifest().installedSkills['commit'].provenance).toBeUndefined();
        expect(readManifest().installedSkills['commit'].source).toBe('github:jinee525/react-component-generator');
    });
    ...
    it('is idempotent — reverting the same ledgerEntryId twice is a no-op on the second call', async () => {
        writeManifest({ commit: makeEntry() });
        const applied = await reconcile({ action: 'mark_local', name: 'commit' }, makeContext());
        const first = await reconcile({ action: 'revert', ledgerEntryId: applied.ledgerEntryId }, makeContext());
        expect(first.noOp).toBe(false);
        const second = await reconcile({ action: 'revert', ledgerEntryId: applied.ledgerEntryId }, makeContext());
    ...
    it('returns a no-op success when no ledger entry matches (nothing to revert)', async () => {
        writeManifest({ commit: makeEntry() });
        const result = await reconcile({ action: 'revert', ledgerEntryId: 'mrc_does_not_exist' }, makeContext());
        expect(result.success).toBe(true);
...
describe('C8 — backup step security', () => {
    ...
    it('assertBackupTargetIsFile refuses a nonexistent path', async () => {
        await expect(assertBackupTargetIsFile(path.join(SKILLSMITH_DIR, 'does-not-exist.json'))).rejects.toBeInstanceOf(ReconcileGuardError);
    });
    it('config.json never appears anywhere in the backups tree after a reconcile', async () => {
        fs.mkdirSync(SKILLSMITH_DIR, { recursive: true });
        fs.writeFileSync(path.join(SKILLSMITH_DIR, 'config.json'), JSON.stringify({ apiKey: 'sk_live_FAKE_DO_NOT_LEAK' }), 'utf-8');
        writeManifest({ commit: makeEntry() });
        await reconcile({ action: 'mark_local', name: 'commit' }, makeContext());
...
//# sourceMappingURL=apply-manifest-reconcile.test.js.map