...
import { appendReconcileLedgerEntry, findReconcileLedgerEntriesFor, readReconcileLedger, readReconcileLedgerResult, removeReconcileLedgerEntry, writeReconcileLedger, } from '../../src/tools/manifest-reconcile-ledger.js';
...
describe('removeReconcileLedgerEntry', () => {
    it('removes the matching entry and is idempotent on a second call (case 6)', async () => {
        ...
        await expect(removeReconcileLedgerEntry(entry.id, { ledgerPath: LEDGER_PATH })).resolves.toBeUndefined();
...
describe('findReconcileLedgerEntriesFor', () => {
    it('filters by (name, client, manifestPath) and sorts most-recent-first (case 7)', async () => {
        await appendReconcileLedgerEntry(makeAppendInput({ name: 'skill-a', manifestPath: '/home/a/.skillsmith/manifest.json' }), { ledgerPath: LEDGER_PATH });
        await new Promise((r) => setTimeout(r, 2));
        await appendReconcileLedgerEntry(makeAppendInput({ name: 'skill-a', manifestPath: '/home/a/.skillsmith/manifest.json' }), { ledgerPath: LEDGER_PATH });
        // Different manifestPath — must NOT be matched (workspace vs. global scope).
        await appendReconcileLedgerEntry(makeAppendInput({
            name: 'skill-a',
            manifestPath: '/home/a/project/.skillsmith/manifest.json',
        }), { ledgerPath: LEDGER_PATH });
        // Different name — must NOT be matched.
        await appendReconcileLedgerEntry(makeAppendInput({ name: 'skill-b', manifestPath: '/home/a/.skillsmith/manifest.json' }), { ledgerPath: LEDGER_PATH });
        const ledger = await readReconcileLedger({ ledgerPath: LEDGER_PATH });
        const matches = findReconcileLedgerEntriesFor(ledger, 'skill-a', 'claude-code', '/home/a/.skillsmith/manifest.json');
        expect(matches).toHaveLength(2);
        // Most-recent-first.
        expect(new Date(matches[0].appliedAt).getTime()).toBeGreaterThanOrEqual(new Date(matches[1].appliedAt).getTime());
    });
});
describe('writeReconcileLedger', () => {
    it('normalizes the written version to CURRENT_LEDGER_VERSION', async () => {
        await writeReconcileLedger({ version: CURRENT_LEDGER_VERSION, entries: [] }, { ledgerPath: LEDGER_PATH });
        const raw = JSON.parse(await fsp.readFile(LEDGER_PATH, 'utf-8'));
        expect(raw.version).toBe(CURRENT_LEDGER_VERSION);
    });
});
//# sourceMappingURL=manifest-reconcile-ledger.test.js.map