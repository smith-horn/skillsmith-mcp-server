...
vi.mock('../../src/middleware/license.js', async (importOriginal) => {
    ...
    return {
        ...actual,
        withLicenseAndQuota: (...args) => mocks.withLicenseAndQuota(...args),
    };
});
import { dispatchAuditTool, isAuditToolName, AUDIT_TOOL_NAMES, } from '../../src/audit-tool-dispatch.js';
// ============================================================================
// Helpers
// ============================================================================
...
describe('SMI-4590 Step 0b — audit-tool-dispatch', () => {
    ...
    describe('dispatchAuditTool routing', () => {
        it('routes skill_audit through withLicenseAndQuota', async () => {
            await dispatchAuditTool('skill_audit', { skillId: 'foo/bar' }, fakeContext(), fakeLicense(), fakeQuota());
        ...
        it('routes skill_pack_audit through withLicenseAndQuota', async () => {
            await dispatchAuditTool('skill_pack_audit', { pack_path: '/tmp/pack' }, fakeContext(), fakeLicense(), fakeQuota());
            expect(mocks.withLicenseAndQuota).toHaveBeenCalledTimes(1);
            const call = mocks.withLicenseAndQuota.mock.calls[0];
        ...
        it('throws "Unknown audit tool: <name>" for unrecognized tool names', async () => {
            await expect(dispatchAuditTool('not_an_audit_tool', {}, fakeContext(), fakeLicense(), fakeQuota())).rejects.toThrow('Unknown audit tool: not_an_audit_tool');
...
//# sourceMappingURL=audit-tool-dispatch.test.js.map