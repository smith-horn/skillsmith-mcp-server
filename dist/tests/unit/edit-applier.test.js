...
import { cid, makeAuditResult, makeEntry, makeSemanticFlag, writeSkillMd, } from './edit-suggester.fixtures.js';
let TEST_HOME;
let ORIGINAL_HOME;
beforeEach(() => {
...
describe('applyRecommendedEdit — registry-rejection regression guard', () => {
    it.each(['narrow_scope', 'reword_trigger_verb'])('rejects synthetic edit with failing-template pattern %s without mutating the file', async (pattern) => {
        ...
        const synthetic = {
            ...
            category: pattern === 'reword_trigger_verb' ? 'claude_md_trigger_overlap' : 'description_overlap',
            pattern,
            filePath,
...
describe('applyRecommendedEdit — apply happy path', () => {
    it('returns the registered summary on apply_with_confirmation success and creates a backup', async () => {
        ...
        expect(applyResult.summary).toBe(`Edited ${edit.filePath} lines ${edit.lineRange.start}-${edit.lineRange.end}. To undo (this session only): call undo_apply with suggestion_id: '${edit.collisionId}'.`);
        ...
        const backupContent = fs.readFileSync(path.join(applyResult.backupPath, backupFiles[0]), 'utf-8');
...
//# sourceMappingURL=edit-applier.test.js.map