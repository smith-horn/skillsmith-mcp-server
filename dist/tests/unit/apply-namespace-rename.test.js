...
import { applyNamespaceRename, applyNamespaceRenameInputSchema, applyNamespaceRenameToolSchema, } from '../../src/tools/apply-namespace-rename.js';
...
function plantSkill(home, identifier) {
 ...
 * for the apply call.
 */
async function seedAuditWithCollision() {
 ...
 */
function resolveObjectShape(schema) {
    const candidate = schema;
    if (candidate.shape)
        return candidate.shape;
    if (typeof candidate.innerType === 'function')
        return candidate.innerType().shape;
    throw new Error('Could not resolve the Zod object shape for applyNamespaceRenameInputSchema — ' +
...
//# sourceMappingURL=apply-namespace-rename.test.js.map