...
import { WARNING_CODES, capTriggerSurface, hashClaudeMdLine, readEnabledPluginIds, splitDescriptionToPhrases, MAX_TRIGGER_PHRASES_PER_SKILL, } from '../../src/utils/local-inventory.helpers.js';
let TEST_HOME;
beforeEach(() => {
...
describe('scanLocalInventory', () => {
    ...
    it('extracts skill kind=skill entries with name from frontmatter', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'skills', 'docker', 'SKILL.md'), `---\nname: docker\ndescription: Docker container management\n---\n# Docker Skill\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
    ...
    it('SMI-6077: scans ~/.cursor/skills (a non-Claude client native directory)', async () => {
        writeFile(path.join(TEST_HOME, '.cursor', 'skills', 'lint-helper', 'SKILL.md'), `---\nname: lint-helper\ndescription: Cursor-only lint helper\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        const skill = result.entries.find((e) => e.identifier === 'lint-helper');
        expect(skill).toBeDefined();
        expect(skill?.kind).toBe('skill');
        expect(skill?.client).toBe('cursor');
        expect(skill?.source_path).toBe(path.join(TEST_HOME, '.cursor', 'skills', 'lint-helper', 'SKILL.md'));
    ...
    it('SMI-6077: scans every CLIENT_IDS client in the same pass, not just Claude Code + agents', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'skills', 'a', 'SKILL.md'), `---\nname: a\n---\nbody\n`);
        writeFile(path.join(TEST_HOME, '.cursor', 'skills', 'b', 'SKILL.md'), `---\nname: b\n---\nbody\n`);
        writeFile(path.join(TEST_HOME, '.copilot', 'skills', 'c', 'SKILL.md'), `---\nname: c\n---\nbody\n`);
        writeFile(path.join(TEST_HOME, '.codeium', 'windsurf', 'skills', 'd', 'SKILL.md'), `---\nname: d\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
    ...
    it('SMI-5456: also scans ~/.agents/skills (the "agents" client, dual-path agent pack)', async () => {
        writeFile(path.join(TEST_HOME, '.agents', 'skills', 'skillsmith-agent', 'SKILL.md'), `---\nname: skillsmith-agent\ndescription: The Skillsmith Agent\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        const skill = result.entries.find((e) => e.kind === 'skill' && e.identifier === 'skillsmith-agent');
        expect(skill).toBeDefined();
        expect(skill?.client).toBe('agents');
        expect(skill?.source_path).toBe(path.join(TEST_HOME, '.agents', 'skills', 'skillsmith-agent', 'SKILL.md'));
    });
    it('SMI-5456: sees the SAME identifier from both .claude/skills and .agents/skills as two distinct entries', async () => {
        ...
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        const matches = result.entries.filter((e) => e.kind === 'skill' && e.identifier === 'skillsmith-agent');
    ...
    it('handles block-scalar description (description: |) via parseYamlFrontmatter', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'skills', 'multi', 'SKILL.md'), `---\nname: multi\ndescription: |\n  First line.\n  Second line.\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
    ...
    it('scans frontmatter-less command files using first body line', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'commands', 'ship.md'), `Ship code: commit, push, PR, merge.\nMore detail follows.\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
    ...
    it('scans agent files with frontmatter description as trigger surface', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'agents', 'reviewer.md'), `---\nname: reviewer\ndescription: Reviews PRs for quality\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
    ...
    it('extracts CLAUDE.md trigger phrases from Trigger phrases heading', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'CLAUDE.md'), `# Project notes\n\n## Trigger phrases\n\n- deploy to staging\n- run the migration\n\n## Other section\n\n- not a trigger\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
    ...
    it('captures high-confidence <!-- skillsmith:trigger --> markers', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'CLAUDE.md'), `# Notes\n\nNormal text.\n\nDeploy to staging <!-- skillsmith:trigger -->\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        const triggers = result.entries.filter((e) => e.kind === 'claude_md_rule');
    ...
    it('does not throw on malformed CLAUDE.md (binary-ish content)', async () => {
        ...
        writeFile(path.join(TEST_HOME, '.claude', 'skills', 'foo', 'SKILL.md'), `---\nname: foo\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        expect(result.entries.some((e) => e.identifier === 'foo')).toBe(true);
    });
    it('populates entry.meta.author from manifest when present', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'skills', 'docker', 'SKILL.md'), `---\nname: docker\n---\nbody\n`);
        writeFile(path.join(TEST_HOME, '.skillsmith', 'manifest.json'), JSON.stringify({
    ...
    it('fresh-install latency: empty home + 5 unmanaged skills completes within 50ms (P-ANTI-2)', async () => {
        for (let i = 0; i < 5; i++) {
            writeFile(path.join(TEST_HOME, '.claude', 'skills', `unmanaged-${i}`, 'SKILL.md'), `---\nname: unmanaged-${i}\ndescription: Test skill ${i}\n---\nbody\n`);
        }
    ...
    it('SMI-6228: tags every Source 1-4 entry with origin "native-client"', async () => {
        ...
        writeFile(path.join(TEST_HOME, '.claude', 'CLAUDE.md'), `## Trigger phrases\n\n- deploy to staging\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
...
describe('scanLocalInventory — Source 5 plugin scan (SMI-6228)', () => {
    ...
    function writePluginSkill(marketplace, plugin, version, skillName, frontmatter) {
        writeFile(path.join(TEST_HOME, '.claude', 'plugins', 'cache', marketplace, plugin, version, 'skills', skillName, 'SKILL.md'), frontmatter);
    }
    it('surfaces a skill from an enabled plugin (enabledPlugins[id] === true), tagged origin "plugin"', async () => {
        ...
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        const pluginSkill = result.entries.find((e) => e.kind === 'skill' && e.identifier === 'shared-name' && e.origin === 'plugin');
    ...
    it('a project skill and an enabled-plugin skill sharing an identifier both appear as distinct entries (real collision, SMI-6228)', async () => {
        writeFile(path.join(TEST_HOME, '.claude', 'skills', 'shared-name', 'SKILL.md'), `---\nname: shared-name\n---\nbody\n`);
        writeSettings({ enabledPlugins: { 'foo@bar': true } });
        writePluginSkill('bar', 'foo', 'abc123', 'shared-name', `---\nname: shared-name\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        const matches = result.entries.filter((e) => e.kind === 'skill' && e.identifier === 'shared-name');
        expect(matches).toHaveLength(2);
        expect(matches.some((e) => e.origin === 'native-client' && e.client === 'claude-code')).toBe(true);
    ...
    it('path-traversal-shaped plugin ids are rejected, never escape the cache root (cross-provider review finding, PR #2581)', async () => {
        ...
        writeFile(path.join(TEST_HOME, '.claude', 'plugins', 'outside', 'v1', 'skills', 'evil', 'SKILL.md'), `---\nname: evil\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME });
        expect(result.entries.some((e) => e.origin === 'plugin')).toBe(false);
...
describe('scanLocalInventory — Source 6 project skills (SMI-6240)', () => {
    ...
    function writeProjectSkill(name, frontmatter) {
    ...
    it('a real collision: the same identifier from an enabled plugin and this project surfaces as two distinct entries (SMI-6240 — the scenario Source 5 alone did not close)', async () => {
        writeProjectSkill('supabase', `---\nname: supabase\n---\nbody\n`);
        writeFile(path.join(TEST_HOME, '.claude', 'settings.json'), JSON.stringify({ enabledPlugins: { 'foo@bar': true } }));
        writeFile(path.join(TEST_HOME, '.claude', 'plugins', 'cache', 'bar', 'foo', 'v1', 'skills', 'supabase', 'SKILL.md'), `---\nname: supabase\n---\nbody\n`);
        const result = await scanLocalInventory({ homeDir: TEST_HOME, projectDir: TEST_PROJECT_DIR });
    ...
    it('a symlinked .claude/skills pointing outside projectDir is rejected, not followed (GPT-5.6-Sol review finding, commit 4a883c9aa)', async () => {
        ...
        fs.mkdirSync(outsideDir, { recursive: true });
        fs.writeFileSync(path.join(outsideDir, 'SKILL.md'), `---\nname: evil\n---\nbody\n`);
        fs.symlinkSync(path.join(TEST_HOME, 'outside-the-project'), path.join(TEST_PROJECT_DIR, '.claude'));
        const result = await scanLocalInventory({ homeDir: TEST_HOME, projectDir: TEST_PROJECT_DIR });
        expect(result.entries.some((e) => e.origin === 'project')).toBe(false);
        expect(result.entries.some((e) => e.identifier === 'evil')).toBe(false);
        expect(result.warnings.some((w) => w.code === WARNING_CODES.PROJECT_SKILLS_SCAN_SKIPPED)).toBe(true);
    });
});
describe('readEnabledPluginIds', () => {
    ...
    it('returns [] when settings.json is missing', () => {
        const warnings = [];
        expect(readEnabledPluginIds(path.join(TEST_HOME, '.claude', 'settings.json'), warnings)).toEqual([]);
        expect(warnings).toEqual([]);
    });
});
...
//# sourceMappingURL=local-inventory.test.js.map