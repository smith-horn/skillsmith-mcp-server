...
import { parseYamlFrontmatter, hasSsrfPattern, hasPathTraversal, validateMetadata, validateNameMatchesDirectory, detectClaudeMdModification, } from '../../src/tools/validate.helpers.js';
...
---`;
    ...
    describe('validateMetadata', () => {
        ...
        it('warns on unknown compatibility value', () => {
            ...
            const errors = validateMetadata(metadata, false);
            expect(errors.some((e) => e.field === 'compatibility' &&
                e.severity === 'warning' &&
                e.message.includes('Unknown compatibility value'))).toBe(true);
        });
        it('does not warn on known compatibility values', () => {
            ...
            const errors = validateMetadata(metadata, false);
            expect(errors.some((e) => e.field === 'compatibility' && e.message.includes('Unknown compatibility'))).toBe(false);
        });
    });
    ...
    describe('validateNameMatchesDirectory', () => {
        ...
        it('still errors on a genuine mismatch given a relative path', () => {
            const errors = validateNameMatchesDirectory('definitely-not-the-cwd-name', './SKILL.md', false);
