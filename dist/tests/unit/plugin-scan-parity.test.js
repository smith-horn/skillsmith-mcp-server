...
function expectParity(settingsPath) {
    const warnings = [];
    const tsResult = readEnabledPluginIdsTs(settingsPath, warnings);
    const mjsResult = readEnabledPluginIdsMjs(settingsPath);
...
describe('readEnabledPluginIds parity (TS vs .mjs mirror, SMI-6229 / ADR-137)', () => {
    it('all-true enabledPlugins', () => {
        const p = writeSettings(JSON.stringify({ enabledPlugins: { 'a@market': true, 'b@market': true } }));
        expectParity(p);
    });
    it('mixed true/false enabledPlugins', () => {
        const p = writeSettings(JSON.stringify({ enabledPlugins: { 'a@market': true, 'b@market': false } }));
        expectParity(p);
    });
...
//# sourceMappingURL=plugin-scan-parity.test.js.map