 ...
 */
async function loadExport(identifier, importMap) {
    const spec = importMap.get(identifier);
    if (!spec) {
        throw new Error(`tool-dispatch.coverage.test.ts: no import found for "${identifier}" in index.ts — ` +
 ...
 */
async function resolveAdvertisedToolNames() {
    ...
    if (!arrayMatch) {
        throw new Error('tool-dispatch.coverage.test.ts: could not find `const toolDefinitions = [...]` in index.ts — ' +
    ...
    for (const rawLine of arrayMatch[1].split('\n')) {
        ...
        if (spread) {
            const builder = (await loadExport(spread[1], importMap));
            for (const tool of builder())
                names.push(tool.name);
            continue;
        }
        const plain = line.match(/^([A-Za-z_$][A-Za-z0-9_$]*),?$/);
        if (plain) {
            const schema = (await loadExport(plain[1], importMap));
            names.push(schema.name);
            continue;
        }
        throw new Error(`tool-dispatch.coverage.test.ts: unrecognized toolDefinitions entry "${line}" — ` +
...
describe('dispatchToolCall — ListTools coverage (SMI-5477 regression guard)', () => {
    ...
    it.each(ADVERTISED_TOOL_NAMES)('dispatchToolCall routes "%s" to a real handler, not the Unknown-tool throw', async (name) => {
        const licenseMiddleware = createLicenseMw();
        const quotaMiddleware = createQuotaMw();
        let unknownToolHit = false;
        try {
            await dispatchToolCall(name, {}, {}, licenseMiddleware, quotaMiddleware);
        }
        catch (error) {
...
//# sourceMappingURL=tool-dispatch.coverage.test.js.map