 ...
 */
function makeBackup(skillName, when, reason) {
...
describe('runBackupGC', () => {
    ...
    it('uses default 14-day retention when env unset (case 8)', async () => {
        ...
        const within = makeBackup('anthropic-ship', new Date(now.getTime() - 13 * MS_PER_DAY), 'namespace-rename');
        // 15 days old — outside default window.
        const outside = makeBackup('anthropic-ship', new Date(now.getTime() - 15 * MS_PER_DAY), 'namespace-rename');
        const result = await runBackupGC({ backupsDir: BACKUPS_ROOT, now });
        expect(result.removed).toBe(1);
        expect(result.kept).toBe(1);
...
//# sourceMappingURL=install.backup-gc.test.js.map