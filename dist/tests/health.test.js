...
import { HealthCheck, checkHealth, createHealthCheck, formatHealthResponse, } from '../src/health/healthCheck.js';
import { ReadinessCheck, checkReadiness, createReadinessCheck, formatReadinessResponse, } from '../src/health/readinessCheck.js';
...
describe('Readiness Check (SMI-740)', () => {
    ...
    describe('ReadinessCheck class', () => {
        ...
        it('should add checks dynamically', async () => {
            const readinessCheck = new ReadinessCheck({ database: db });
            readinessCheck.addCheck('dynamic-check', async () => ({ ok: true, details: { added: 'dynamically' } }), false);
            const result = await readinessCheck.check();
