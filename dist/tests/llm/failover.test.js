...
import { LLMFailoverChain, createLLMFailoverChain, DEFAULT_LLM_FAILOVER_CONFIG, } from '../../src/llm/failover.js';
describe('LLMFailoverChain', () => {
    ...
    describe('Request Completion', () => {
        ...
        it('should throw if not initialized', async () => {
            ...
            await expect(uninitializedChain.complete({
                messages: [{ role: 'user', content: 'Hello' }],
            })).rejects.toThrow('not initialized');
        });
        it('should throw if disabled', async () => {
            const disabledChain = await createLLMFailoverChain({ enabled: false });
            await expect(disabledChain.complete({
                messages: [{ role: 'user', content: 'Hello' }],
