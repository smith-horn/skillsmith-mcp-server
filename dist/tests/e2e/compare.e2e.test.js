...
import { executeCompare } from '../../src/tools/compare.js';
...
// Test configuration
const TEST_DIR = join(tmpdir(), `skillsmith-e2e-compare-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`);
...
describe('E2E: skill_compare tool', () => {
    ...
    describe('Basic Comparison', () => {
        it('should compare two skills', async () => {
            ...
            const { result: _rawResult, durationMs } = await measureAsync('compare:two', 'skill_compare (two)', () => executeCompare(input, context));
    ...
    describe('Performance Baselines', () => {
        it('should complete comparison in reasonable time', async () => {
            const { durationMs } = await measureAsync('compare:baseline', 'skill_compare baseline', () => executeCompare({
                skill_a: 'anthropic/commit',
                skill_b: 'community/commitizen',
            }, context));
...
//# sourceMappingURL=compare.e2e.test.js.map