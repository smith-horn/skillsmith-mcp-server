...
describe('E2E: Skill Discovery Flow', () => {
    ...
    beforeAll(() => {
        ...
        const testDir = join(tmpdir(), `skillsmith-e2e-test-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`);
...
describe('E2E: Data Quality Validation', () => {
    ...
    let testDbPath;
    beforeAll(() => {
        const testDir = join(tmpdir(), `skillsmith-e2e-quality-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`);
