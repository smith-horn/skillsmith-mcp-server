...
import { createToolContext, closeToolContext } from '../../src/context.js';
import { executeSuggest } from '../../src/tools/suggest.js';
...
// Test configuration
const TEST_DIR = join(tmpdir(), `skillsmith-e2e-suggest-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`);
...
function createMockProject() {
    ...
    // Create a test file
    writeFileSync(join(TEST_PROJECT_DIR, 'src', 'App.test.tsx'), `import { render } from '@testing-library/react';
import App from './App';
...
describe('E2E: skill_suggest tool', () => {
    ...
    describe('Basic Suggestions', () => {
        it('should return suggestions for project path', async () => {
            ...
            const { result: _rawResult, durationMs } = await measureAsync('suggest:basic', 'skill_suggest (basic)', () => executeSuggest(input, context));
    ...
    describe('Quota Behaviour', () => {
        ...
        it('should not throttle consecutive calls from the same session', async () => {
            ...
            const result1 = await executeSuggest({ project_path: TEST_PROJECT_DIR, session_id: sessionId, limit: 3 }, context);
            const result2 = await executeSuggest({ project_path: TEST_PROJECT_DIR, session_id: sessionId, limit: 3 }, context);
    ...
    describe('CRITICAL: Hardcoded skillDatabase Detection', () => {
        ...
        it('should use database skills not hardcoded skillDatabase', async () => {
            ...
            if (fromHardcoded.length > 0) {
                ...
                queueIssue(failure);
                expect.fail(`HARDCODED skillDatabase detected! Suggestions ${fromHardcoded.join(', ')} are from hardcoded list, not database.`);
            }
        });
    ...
    describe('Performance Baselines', () => {
        it('should complete suggestion in reasonable time', async () => {
            const { durationMs } = await measureAsync('suggest:baseline', 'skill_suggest baseline', () => executeSuggest({
                project_path: TEST_PROJECT_DIR,
                current_file: 'src/App.test.tsx',
    ...
    describe('Error Handling', () => {
        it('should handle missing project path', async () => {
            await expect(executeSuggest({ project_path: '' }, context)).rejects.toThrow();
        });
        ...
        it('should handle invalid limit', async () => {
            await expect(executeSuggest({ project_path: TEST_PROJECT_DIR, limit: 100 }, context)).rejects.toThrow();
        });
    });
});
//# sourceMappingURL=suggest.e2e.test.js.map