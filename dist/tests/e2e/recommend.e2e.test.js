...
// Test configuration
const TEST_DIR = join(tmpdir(), `skillsmith-e2e-recommend-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`);
...
describe('E2E: skill_recommend tool', () => {
    ...
    describe('Basic Recommendations', () => {
        it('should return recommendations without installed skills', async () => {
            ...
            const { result: _rawResult } = await measureAsync('recommend:empty', 'skill_recommend (no installed)', () => executeRecommend(input, context));
