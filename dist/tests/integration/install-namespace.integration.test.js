...
beforeEach(async () => {
    ...
    vi.spyOn(embeddings.EmbeddingService.prototype, 'embed').mockImplementation(async () => new Float32Array(384));
    ...
    const realModule = await vi.importActual('../../src/utils/local-inventory.js');
    vi.mocked(scanLocalInventory).mockImplementation((opts) => realModule.scanLocalInventory({ ...opts, homeDir: TEST_HOME }));
});
afterEach(() => {
 ...
 */
function plantShadow(dirName, identifier, author) {
    const dir = path.join(SKILLS_DIR, dirName);
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'SKILL.md'), ['---', `name: ${identifier}`, `author: ${author}`, '---', `# ${identifier}`, '', 'body'].join('\n'), 'utf-8');
    return dir;
}
...
function skillSuggestion(args) {
    ...
    return {
        collisionId: (args.collisionId ?? `cid-${args.identifier}`),
        entry,
...
describe('install namespace integration', () => {
    it('case 1: preventative + collision → blocks install; on-disk state unchanged', async () => {
        ...
        expect(outcome.resultPatch.pendingCollision.suggestedRename.suggested).toBe('anthropic-code-helper');
    ...
    it('case 2: two-step agent flow — block → applyRename (Wave 4 stub) → gate proceeds', async () => {
        ...
        expect(blocked.resultPatch.pendingCollision.suggestedRename.suggested).toBe('anthropic-code-helper');
    ...
    it('case 10: end-to-end — gate-block → applyRename → gate-proceed; ledger has 1 entry', async () => {
        ...
        const auditId = newAuditId();
        // A: gate blocks.
        expect((await runNamespaceGate({ candidate: c, mode: 'preventative', tier: 'community' })).decision).toBe('block');
...
//# sourceMappingURL=install-namespace.integration.test.js.map