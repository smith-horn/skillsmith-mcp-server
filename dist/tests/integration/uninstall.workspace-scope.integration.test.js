...
describe('ADR-139 (SMI-6274 Wave 4): uninstall_skill MCP tool workspace-scope wiring', () => {
    ...
    afterEach(async () => {
        ...
        if (ORIGINAL_SKILLSMITH_SCOPE === undefined)
            delete process.env['SKILLSMITH_SCOPE'];
        else
            process.env['SKILLSMITH_SCOPE'] = ORIGINAL_SKILLSMITH_SCOPE;
    ...
    it('scope: "workspace" removes the workspace-scoped copy and leaves an identically-named global copy untouched', async () => {
        ...
        await writeFile(path.join(globalSkillDir, 'SKILL.md'), `---\nname: dual-scope-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
    ...
    it('scope: "global" (the default) removes only the global copy, leaving an identically-named workspace copy untouched', async () => {
        ...
        const result = await uninstallSkill({ skillName: 'dual-scope-skill-2', force: true, cwd: neutralCwd }, context);
        expect(result.success).toBe(true);
    ...
    it("GPT-5.6-Sol PR review round 2: a non-canonical-client, workspace-scoped uninstall does NOT remove a same-named skill's canonical --also-link fan-out destinations", async () => {
        ...
        vi.resetModules();
        const { ManifestManager } = await import('@skillsmith/core');
        const { getLinkManifestPath, saveManifest: saveLinkManifest } = await import('@skillsmith/core/install');
        ...
        await writeFile(path.join(canonicalSkillDir, 'SKILL.md'), `---\nname: link-guard-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
        const globalManifestPath = path.join(homeDir, '.skillsmith', 'manifest.json');
        await new ManifestManager(globalManifestPath).save({
            version: '1.0.0',
            installedSkills: {
        ...
        await writeFile(path.join(fanOutDestDir, 'SKILL.md'), `---\nname: link-guard-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
        ...
        await writeFile(path.join(cursorWorkspaceSkillDir, 'SKILL.md'), `---\nname: link-guard-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
        const workspaceManifestPath = path.join(workspaceDir, '.skillsmith', 'manifest.json');
        await new ManifestManager(workspaceManifestPath).save({
            version: '1.0.0',
            installedSkills: {
        ...
        const linkManifest = JSON.parse(linkManifestRaw);
    ...
    it("GPT-5.6-Sol PR review round 3: a canonical-client, WORKSPACE-scoped uninstall does NOT remove the unrelated GLOBAL canonical install's --also-link fan-out destinations", async () => {
        ...
        const fanOutDestDir = path.join(homeDir, '.codeium', 'windsurf', 'skills', 'scope-link-guard-skill');
        await mkdir(fanOutDestDir, { recursive: true });
        await writeFile(path.join(fanOutDestDir, 'SKILL.md'), `---\nname: scope-link-guard-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
        await saveLinkManifest({
            version: 1,
    ...
    it('SMI-6529: a canonical-client, GLOBAL-scoped uninstall surfaces a leftover interrupted-refresh backup warning from removeLinks in result.warning', async () => {
        ...
        const fanOutDestDir = path.join(homeDir, '.codeium', 'windsurf', 'skills', 'leftover-warning-skill');
        await mkdir(fanOutDestDir, { recursive: true });
        await writeFile(path.join(fanOutDestDir, 'SKILL.md'), `---\nname: leftover-warning-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
        await saveLinkManifest({
            version: 1,
        ...
        const leftoverBackupDir = path.join(homeDir, '.codeium', 'windsurf', 'skills', '.leftover-warning-skill.skillsmith-backup-AbC123');
        await mkdir(path.join(leftoverBackupDir, 'original'), { recursive: true });
        await writeFile(path.join(leftoverBackupDir, 'original', 'SKILL.md'), `---\nname: leftover-warning-skill\ndescription: test\n---\n${SKILL_MD_BODY}`, 'utf-8');
        const { createToolContext } = await import('../../src/context.js');
        const context = createToolContext({
            dbPath: ':memory:',
            apiClientConfig: { offlineMode: true },
        ...
        const result = await uninstallSkill({ skillName: 'leftover-warning-skill', force: true, cwd: neutralCwd }, context);
        expect(result.success).toBe(true);
...
//# sourceMappingURL=uninstall.workspace-scope.integration.test.js.map