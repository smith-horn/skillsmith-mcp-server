...
import { baseSpawnEnv, connectHarness, createIsolatedHome, ensureDistBuilt, } from './agent-harness-sim.helpers.js';
/**
...
describe('SMI-6472 Wave 2 — tools/list wire response carries title + annotations', () => {
    ...
    beforeAll(async () => {
        ...
        connection = await connectHarness({ name: 'tools-list-annotations-test', version: '1.0.0' }, env);
    ...
    function findTool(name) {
        ...
        if (!tool) {
            throw new Error(`Expected tool "${name}" in tools/list response but it was not present. ` +
    ...
    it('every tool has an `annotations` object with boolean readOnlyHint + destructiveHint', () => {
        const offenders = tools
            ...
            .map((tool) => tool.name);
        expect(offenders, `tools missing boolean readOnlyHint/destructiveHint annotations: ${offenders.join(', ')}`).toEqual([]);
    });
    it('no tool sets both readOnlyHint:true and destructiveHint:true (contradictory)', () => {
        const offenders = tools
            ...
            .map((tool) => tool.name);
        expect(offenders, `tools with contradictory readOnlyHint:true + destructiveHint:true: ${offenders.join(', ')}`).toEqual([]);
    });
    it('search: readOnlyHint true, destructiveHint false, title "Search Skills"', () => {
        const tool = findTool('search');
        expect(tool.annotations, `search.annotations was: ${JSON.stringify(tool.annotations)}`).toMatchObject({
            readOnlyHint: true,
    ...
    it('uninstall_skill: readOnlyHint false, destructiveHint true, title "Uninstall Skill"', () => {
        ...
        expect(tool.annotations, `uninstall_skill.annotations was: ${JSON.stringify(tool.annotations)}`).toMatchObject({ readOnlyHint: false, destructiveHint: true });
        expect(tool.title, `uninstall_skill.title was: ${JSON.stringify(tool.title)}`).toBe('Uninstall Skill');
    });
    it('skill_rescan: readOnlyHint false, destructiveHint false (additive-only — proves values are not just a blanket default)', () => {
        ...
        expect(tool.annotations, `skill_rescan.annotations was: ${JSON.stringify(tool.annotations)}`).toMatchObject({ readOnlyHint: false, destructiveHint: false });
    });
    it('title is top-level on the tool, not nested inside annotations', () => {
        const tool = findTool('search');
        expect(tool.title, `search.title was: ${JSON.stringify(tool.title)}`).toBe('Search Skills');
        expect(tool.annotations?.title, `search.annotations.title should be undefined but was: ${JSON.stringify(tool.annotations?.title)}`).toBeUndefined();
    });
});
//# sourceMappingURL=tools-list-annotations.integration.test.js.map