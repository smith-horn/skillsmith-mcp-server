...
import { createTestFilesystem } from './setup.js';
...
describe('SMI-5639: shutdown persistence — exact repro (integration, no subprocess)', () => {
    ...
    beforeEach(async () => {
        ...
        dbPath = path.join(os.tmpdir(), `skillsmith-shutdown-persistence-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
        vi.clearAllMocks();
    ...
    it('persists skill_dependencies rows written before shutdown into a fresh connection reopened after close()', async () => {
        ...
        try {
            ...
            const result = await installSkill(installInputSchema.parse({ skillId: TEST_SKILL_ID, skipScan: true, force: true }), context);
...
//# sourceMappingURL=shutdown-persistence.integration.test.js.map