...
import { computeAcceptKey, computeStoreDigest, findingFingerprint, } from '../../src/audit/security-acceptance.js';
...
function mintDeadPid() {
    ...
    throw new Error('[security-acceptance-lost-update.test] could not mint a deterministically-dead PID');
}
...
async function waitForAsync(predicate, label, 
...
function seedPreexistingStore(acceptancePath) {
    ...
    const shell = { version: 1, revision: 5, records: [record] };
    const storeDigest = computeStoreDigest(shell);
    writeFileSync(acceptancePath, JSON.stringify({ ...shell, storeDigest }, null, 2));
    return record;
}
describe('security-acceptance lost update under a forced stale reclaim (§9 H-9)', () => {
    it('no child that reports ok:true ever loses its acceptance; revision and lock state are consistent', async () => {
        ...
        for (let iter = 0; iter < ITERATIONS; iter++) {
            const dir = path.join(os.tmpdir(), `sec-acceptance-lost-update-${iter}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
            ...
            const outcomes = [c1, c2].map((c) => {
                ...
                try {
                    ...
                    throw new Error(`[iteration ${iter}] child produced unparseable stdout: ${JSON.stringify(c)}`);
                }
            });
            const successes = outcomes.filter((o) => o.ok);
            const finalRaw = JSON.parse(readFileSync(acceptancePath, 'utf-8'));
            const finalKeys = new Set(finalRaw.records.map((r) => r.acceptKey));
            for (const s of successes) {
                expect(finalKeys.has(s.acceptKey), `iteration ${iter}: lost acceptKey ${s.acceptKey}`).toBe(true); // (a) INV-7
            }
