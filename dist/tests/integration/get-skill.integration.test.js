...
import { createTestDatabase, TEST_SKILLS_STATS } from './setup.js';
describe('Get Skill Tool Integration Tests', () => {
    ...
    describe('Retrieve Skill by Repository URL', () => {
        it('should find skill by repo URL', () => {
            const skill = dbContext.skillRepository.findByRepoUrl('https://github.com/anthropics/claude-code-skills/commit');
            expect(skill).not.toBeNull();
            expect(skill?.id).toBe('anthropic/commit');
        });
        it('should return null for non-existent repo URL', () => {
            const skill = dbContext.skillRepository.findByRepoUrl('https://github.com/fake/nonexistent-repo');
            expect(skill).toBeNull();
        });
    });
    describe('Skill Tags', () => {
...
//# sourceMappingURL=get-skill.integration.test.js.map