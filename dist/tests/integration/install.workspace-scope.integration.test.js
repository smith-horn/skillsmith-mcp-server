...
vi.mock('../../src/tools/install.namespace-gate.js', async (importActual) => {
    ...
    return {
        ...actual,
        runNamespaceGate: vi.fn(async (input) => ({
            decision: 'proceed',
...
describe('ADR-139 (SMI-6274 Wave 4): install_skill MCP tool workspace-scope wiring', () => {
    ...
    beforeEach(async () => {
        ...
        vi.mocked(ioModule.fetchFromGitHub).mockResolvedValue(SKILL_MD);
        vi.mocked(ioModule.fetchAndScanOptionalFiles).mockResolvedValue({ configWarnings: [], failedScans: [], filesToWrite: [] });
    });
    afterEach(async () => {
        vi.restoreAllMocks();
    ...
    it('SKILLSMITH_SCOPE=workspace + cwd installs into the workspace directory, never the global one', async () => {
        ...
        const workspaceManifestRaw = await readFile(path.join(workspaceDir, '.skillsmith', 'manifest.json'), 'utf-8');
        const workspaceManifest = JSON.parse(workspaceManifestRaw);
    ...
    it('SMI-6529: --also-link fan-out surfaces a leftover interrupted-refresh backup warning in tips', async () => {
        ...
        const backupDir = path.join(homeDir, '.cursor', 'skills', '.workspace-scope-test-skill.skillsmith-backup-AbC123');
...
//# sourceMappingURL=install.workspace-scope.integration.test.js.map