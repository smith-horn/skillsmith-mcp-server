...
describe('SMI-6343: $HOME sandbox is inherited by vitest.config.integration.ts', () => {
    ...
    // runs this normally.
    it.skipIf(!CORE_IS_FROM_THIS_TREE)('ManifestManager refuses a manifest path under the captured real home (runtime guard)', async () => {
        ...
        try {
            ...
            await expect(manager.save({ version: '1.0.0', installedSkills: {} })).rejects.toThrow(/SMI-6343/);
            ...
            if (previousRealHome === undefined)
                delete process.env.SKILLSMITH_TEST_REAL_HOME;
            else
                process.env.SKILLSMITH_TEST_REAL_HOME = previousRealHome;
...
//# sourceMappingURL=home-sandbox.integration.test.js.map