...
import { createDatabaseAsync, initializeSchema, closeDatabase, SkillRepository, } from '@skillsmith/core';
...
describe('SMI-6472 Wave 3 — outputSchema + structuredContent on search/get_skill/skill_validate', () => {
    ...
    function requireConnection() {
        if (!connection)
            throw new Error('connectHarness() never completed — see beforeAll failure');
        return connection;
    }
    ...
    describe('tools/list wire shape', () => {
        ...
        it('a sampled tool that should NOT declare an outputSchema (skill_outdated) does not', () => {
            ...
            expect(tool.outputSchema, `skill_outdated.outputSchema was: ${JSON.stringify(tool.outputSchema)}`).toBeUndefined();
        });
    });
    describe('callTool round-trips structuredContent, validated by the real SDK Client', () => {
        ...
        it('skill_validate: validates a real temp skill directory and returns matching structuredContent', async () => {
            ...
            expect(result.isError).toBeFalsy();
            expect(result.structuredContent, 'skill_validate did not return structuredContent').toBeDefined();
            const structured = result.structuredContent;
            ...
            expect(structured.path.endsWith('SKILL.md')).toBe(true);
        });
    });
});
//# sourceMappingURL=tools-output-schema.integration.test.js.map