...
`;
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (e) => e.field === 'name' && e.severity === 'error' && e.message.includes('lowercase letter'))).toBe(true);
        });
...
# Matching Skill
`;
            const skillPath = await createMockInstalledSkill(ctx.skillsDir, 'matching-skill', matchingContent);
            const result = await executeValidate({
                skill_path: skillPath,
            });
            expect(result.valid).toBe(true);
...
# Mismatched Skill
`;
            const skillPath = await createMockInstalledSkill(ctx.skillsDir, 'actual-dir-name', mismatchContent);
            const result = await executeValidate({
                skill_path: skillPath,
            });
            expect(result.valid).toBe(false);
            ...
            (e) => e.field === 'name' &&
                ...
                e.message.includes('actual-dir-name'))).toBe(true);
        });
...
`;
            const skillDir = await createMockInstalledSkill(ctx.skillsDir, 'file-mode-dir', mismatchContent);
            ...
            (e) => e.field === 'name' &&
                ...
                e.message.includes('file-mode-dir'))).toBe(true);
        });
    });
