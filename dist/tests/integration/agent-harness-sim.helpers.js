 ...
 */
export async function connectHarness(clientInfo, env, options = {}) {
    ...
    const cleanupTransport = async () => {
        try {
            ...
            console.error(`[connectHarness] cleanupTransport() failed (pid=${transport.pid ?? 'unassigned'}): ` +
    ...
    const timeoutPromise = new Promise((_, reject) => {
        timeoutHandle = setTimeout(() => {
            ...
            const stderrNote = stderr.length > 0
                ? stderr
                : '<empty — process likely still in Node bootstrap under contention, not a hang>';
            reject(new HarnessConnectTimeoutError(`connectHarness() exceeded its ${timeoutMs}ms liveness budget for ` +
                `clientInfo=${JSON.stringify(clientInfo)} (elapsedMs=${elapsedMs}, ` +
                `pid=${transport.pid ?? 'unassigned'}). stderr at timeout: ${stderrNote}`, { elapsedMs, pid: transport.pid, stderr }));
        }, timeoutMs);
    });
