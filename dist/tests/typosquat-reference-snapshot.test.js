...
describe('bundled typosquat reference snapshot (SMI-6033 Wave 1, Gap 7)', () => {
    ...
    it('is NOT empty — an empty names array silently disables skill_validate + skill_rescan typosquat checks', () => {
        ...
        expect(names.length, 'Snapshot shipped with zero reference names — regenerate with ' +
            '`varlock run -- npx tsx scripts/generate-typosquat-snapshot.ts`').toBeGreaterThan(0);
    });
    it('carries a non-null, parseable, non-future `generatedAt`', () => {
        ...
        expect(Number.isNaN(parsedMs), `\`generatedAt\` is unparseable: ${String(generatedAt)}`).toBe(false);
    ...
    it('contains only non-blank, lowercase, de-duplicated names', () => {
        const names = loadSnapshot().names;
...
//# sourceMappingURL=typosquat-reference-snapshot.test.js.map