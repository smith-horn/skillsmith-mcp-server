...
if (skipInPrePush) {
    console.warn('[SMI-5548] skipping shutdown-persistence subprocess test in pre-push (dist absent; covered by CI)');
}
describe.skipIf(skipInPrePush)('SMI-5639 shutdown persistence — subprocess SIGTERM', () => {
    ...
    it('creates and persists the WASM database file to disk on SIGTERM shutdown', async () => {
        tmpDbPath = path.join(os.tmpdir(), `skillsmith-shutdown-subprocess-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
...
//# sourceMappingURL=shutdown-persistence-subprocess.test.js.map