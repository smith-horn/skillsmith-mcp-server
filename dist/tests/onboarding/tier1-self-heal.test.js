...
import { maybeInstallMissingTier1Skills, readTier1Status, writeTier1Status, TIER1_STATUS_FILE, isTier1AutoInstallDisabled, } from '../../src/onboarding/tier1-self-heal.js';
...
function attributionOf(name) {
    const skill = TIER1_SKILLS.find((s) => s.name === name);
    if (!skill)
        throw new Error(`Unknown Tier-1 skill: ${name}`);
    return skill.id.split('/')[0];
}
function successResult(name) {
    return { success: true, skillId: name, installPath: `/fake/path/${name}` };
}
function failureResult(name, error = 'install failed') {
    return { success: false, skillId: name, installPath: '', error };
}
function alreadyInstalledResult(name) {
    return {
        success: false,
        skillId: name,
        installPath: '',
...
describe('Tier-1 Self-Heal (SMI-5582)', () => {
    let statusFileExistedBefore;
    let originalStatusContent;
    let originalDisableEnv;
    beforeEach(() => {
    ...
    function seedStatusFile(status) {
        if (status === undefined) {
            if (existsSync(TIER1_STATUS_FILE))
                rmSync(TIER1_STATUS_FILE);
            return;
        }
    ...
    describe('maybeInstallMissingTier1Skills()', () => {
        ...
        it('throttle expired: lastAttempt >24h ago retries the missing skill(s)', async () => {
            ...
            expect(installSkillMock).toHaveBeenCalledWith(expect.objectContaining({ skillId: 'getsentry/code-review' }), toolContext);
            const persisted = readTier1Status();
            expect(persisted.installed.sort()).toEqual([...SKILL_NAMES].sort());
        });
...
//# sourceMappingURL=tier1-self-heal.test.js.map